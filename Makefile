...
# Track mutant lists join here as their task merges (M1, Q1); each gates on INFRX_MUTANTS.
api-mutants:
	cd $(API) && INFRX_MUTANTS=all uv run --frozen pytest -q tests/contracts/test_mutants.py tests/m/test_mutants.py tests/q/test_mutants.py
