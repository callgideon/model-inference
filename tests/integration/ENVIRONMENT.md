...
| `app-e2e.sh` | preflight (profile `app-e2e`: node/pnpm, python/uv, Docker, the E2 images, namespace `e4b`) → `make console-test console-lint console-typecheck` → E3A's browser journey `tests/integration/app/runner.py --out <out>/e3a` (the E3C world as namespace `e4b`, the App built and started on 127.0.0.1:56870 with local-only values, the Playwright journey with `PLAYWRIGHT_BROWSERS_PATH` pointing at a scratch directory that holds `chromium-headless-shell`: `PLAYWRIGHT_BROWSERS_PATH=<dir> pnpm --dir apps/app exec playwright install chromium-headless-shell`; the runner reports NOT RUN with the reason when the browsers are missing, and its `verdict.json["verdict"]` counts only if it agrees with the runner's exit code, else INVALID) | worst stage; APP-LOCAL is never claimed by this gate alone |
...
| `e4b` | E4B's certify block (E2 layout +1300) and E3A's App journey inside it | 56800–56899 (edge 56860, control 56861, App 56870) | `infrx-e4b-*` | `app-e2e.sh` browser journey (`tests/integration/app/runner.py`) and `certify.py --box`; never both at once |
...
- 2026-09-26: `app-e2e` profile declares python/uv/Docker, the E2 images and namespace `e4b`; the gate ends with E3A's browser journey (E3A-PREP merge, wiring E3A-WR-3); no behaviour change to the other gates.
