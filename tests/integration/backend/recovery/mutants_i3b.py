...
MUTANTS += (
    ...
    # RS-1 residual: the identity is libpq's resolution, not the conninfo's text
    Mutant("i3bm80", "RS-1r: every loopback spelling is the same host", PGRESTORE,
           '    host = "127.0.0.1" if host in LOOPBACK else host\n', "    host = host\n",
           RESTORE, "bk01e_c and localhost"),
    Mutant("i3bm81", "RS-1r: an omitted port is libpq's 5432", PGRESTORE,
           '"port": value("port", "PGPORT") or "5432"', '"port": value("port", "PGPORT")',
           RESTORE, "bk01e_c and omitted_port"),
    Mutant("i3bm82", "RS-1r: hostaddr names the host when host is empty", PGRESTORE,
           '    host = value("host", "PGHOST") or value("hostaddr", "PGHOSTADDR")\n',
           '    host = value("host", "PGHOST")\n', RESTORE, "bk01e_c and hostaddr"),
    Mutant("i3bm83", "RS-1r: the PG* environment fills what the conninfo omits", PGRESTORE,
           '        return str(parsed.get(key) or os.environ.get(env) or "")',
           '        return str(parsed.get(key) or "")', RESTORE, "bk01e_c and environment"),
    ...
    Mutant("i3bm103", "RST-3: the check compares column privileges", PGRESTORE,
           '    "column_acls": "select n.nspname', '    "_column_acls": "select n.nspname',
           RESTORE, "bk01f and column_acls", layer=2),
...
MUTANTS += (
    ...
    Mutant("i3bm58", "RS-4r: a timed-out client is re-raised by name, not as TimeoutExpired",
           PGRESTORE, "    except subprocess.TimeoutExpired:\n", "    except OSError:\n",
           RUNBOOK_CASES, "rb07"),
    Mutant("i3bm59", "RS-4r: the argv-bearing TimeoutExpired is not chained", PGRESTORE,
           'timed out after 1800 s") from None', 'timed out after 1800 s")',
           RUNBOOK_CASES, "rb07"),
