...
const T = {
  ...
  rLedger: "U1R-R02 the ledger is the caller's own consumer_credit_ledger page on D10's cursor, never a caller-named wallet",
  ...
  uWindow: "U1R-U05 the window is [now - range, now) and goes to consumer_jobs with the model and key; unknown URL values are no filter",
  uHrefs: "U1R-U06 every href is computed here: pages carry the filters, applying filters resets the cursor, rows link to their detail",
...
const MUTANTS = [
  ...
  // U1R-M19 (actor re-selected), M26/M27 (the client-side window cut) and M28 (withJobRange) retired
  // with the code: the ledger RPC returns no actor, and consumer_jobs applies the window (0024).
  { id: "U1R-M29", what: "the next page resumes from the first row instead of the last", file: READS,
    find: "cursors[shown.length - 1]", replace: "cursors[0]", cases: [T.rJobs, T.rLedger] },
  // APP-0024-WIRE: the usage filters reach consumer_jobs' p_model, p_key_id, p_from, p_to (U1R WR-3(a)).
  { id: "U1R-M34", what: "p_to is ignored (the window never ends)", file: READS,
    find: "  if (page.to) args.p_to = page.to;\n", replace: "", cases: [T.rFilters] },
  { id: "U1R-M35", what: "the window's bounds are passed in the wrong order", file: READS,
    find: "  if (page.from) args.p_from = page.from;\n  if (page.to) args.p_to = page.to;",
    replace: "  if (page.from) args.p_to = page.from;\n  if (page.to) args.p_from = page.to;", cases: [T.rFilters] },
  { id: "U1R-M36", what: "the model and key filters are passed to each other's parameter", file: READS,
    find: "  if (page.model) args.p_model = page.model;\n  if (page.keyId) args.p_key_id = page.keyId;",
    replace: "  if (page.model) args.p_key_id = page.model;\n  if (page.keyId) args.p_model = page.keyId;", cases: [T.rFilters] },
  { id: "U1R-M37", what: "an unset filter is sent anyway (the unfiltered call is no longer 0021's)", file: READS,
    find: "  if (page.model) args.p_model = page.model;", replace: "  args.p_model = page.model as string;", cases: [T.rFilters] },
  { id: "U1R-M38", what: "the page never sends the window's end", file: JOBS,
    find: "    to: filters.range === \"all\" ? null : instantOf(now),", replace: "    to: null,", cases: [T.uWindow] },
  { id: "U1R-M39", what: "a key that is not a uuid (the form's \"all\") reaches the database", file: JOBS,
