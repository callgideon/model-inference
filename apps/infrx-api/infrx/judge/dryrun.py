...
`JUDGE_MODE` defaults to `dry_run` and `JUDGE_LIVE_BUDGET_USD` to `0` (08 §5), so
this is the only judge path that runs at all until an operator changes both. It
reaches the trace source and nothing else: no provider client, no `anthropic` import,
no egress. `tests/j/test_no_egress.py` proves that in a subprocess rather than
...
from .cost import (APPROVED_RATES, CostEstimate, RateTable, TokenCeilings,
                   estimate_worst_case, live_submission_allowed)
from .rubric import MARLIN_VIDEO_V1, Rubric
from .sampling import (DEFAULT_DESIGN, CalibrationDesign, CandidateSource, Selection, select)
...
class DryRunPlan:
    ...
    def judge_run(self, *, run_id: str, consent: ConsentSnapshot, model_revision: str,
        ...
        rubric version is the integer the record demands.
        """
...
async def plan_dry_run(source: CandidateSource, *, org_id: str, consent: ConsentSnapshot,
    ...
    """Select, price and report. Raises `ConsentMissing` before reading anything the
    org has not consented to evaluating, and `NotFound` on a tenant mismatch."""
    candidates = await source.candidates(org_id, since=since, limit=limit)
