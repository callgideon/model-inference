"""Trace capture accounting: the one implementation of R27/R37/R42 (F2R item 3).
...
`traces.spool.SpoolTraceSink` both subclass these two classes, and neither imports the
other. No test hook lives here.
"""
...
class TraceCaptureBase:
    ...
    def __init__(self, sink: TraceSinkBase, request_id: str, org_id: str, mode: TraceMode,
    ...
    def __enter__(self) -> TraceCaptureBase:
        return self
...
class TraceSinkBase:
    """`ports.TraceSink` accounting: budgets, the queue and the loss table (R27/R37/R42).

    The clock is **required** (T1 ruling 7): a sink on a clock that never advances never
    fsyncs and never reaps. There is no crash simulation and no failure injection here;
    those are test doubles and live with the fakes and the tests.
    """

    def __init__(self, clock, *, limits: PilotSettings = DEFAULTS) -> None:
        if clock is None:
            raise ValueError("a trace sink needs the clock it reads; None is not one")
        self.clock = clock
        self.limits = limits
        ...
        self.captures: list[TraceCaptureBase] = []
        self._last_fsync: datetime = self.clock.now()

    capture_class = TraceCaptureBase

    @staticmethod
    def metadata_charge(envelope: TraceEnvelope, serialized: int) -> int:
        """What one queued record costs the metadata reserve: `max(declared, serialized)`."""
        return max(envelope.metadata_bytes, serialized)
    ...
    def open(self, request_id: str, org_id: str, mode: TraceMode,
        ...
        capture = self.capture_class(self, request_id, org_id, mode, deadline_at=deadline_at,
                                   no_op=no_op)
    ...
    def _enqueue(self, envelope: TraceEnvelope, *, charged: int,
                 capture: TraceCaptureBase | None = None,
                 serialized: int | None = None) -> TraceOfferResult:
        ...
        if serialized is None:
            try:
                serialized = len(codec.compact_bytes(envelope))
            except Exception:                 # noqa: BLE001 - R37: never raise into the
                # request path. Only the charge is decided here: whether an unwritable
        ...
        for reason in (TraceLossReason.queue_full if len(self.queued) >= self.limits.trace_queue_max
                       ...
                       if self.metadata_bytes + metadata
