...
def resolve_wallet(auth: AuthContextV2, wallet: WalletRef | None) -> WalletRef:
    ...
    * an operator credential spends nothing — operator actions are audited grants
    ...
    # The wallet *kind* is checked explicitly, before ownership. The record
    # validators do make a wrong-kind wallet with matching owners unconstructible
    # through `model_validate` — but pydantic's `model_copy(update=)` and
    # `model_construct` skip mode="after" validators, and `model_copy(update=)` is
    # this tree's own idiom. A provider_dev wallet copied with a consumer's
    # `owner_user_id`/`personal_org_id` (or the reverse) passes every ownership
    # equality, so the kind check is load-bearing, not redundant (F2P review B1).
    if auth.audience is CredentialAudience.consumer:
        if wallet.kind is not WalletKind.consumer:
            raise errors.Forbidden("a consumer credential spends a consumer wallet only")
...
def pin_admission(*, auth: AuthContextV2, requested_model: str,
    ...
    * a card that prices a different deployment or serving revision -> refused,
    ...
    if deployment.visibility is Visibility.private:
        # A private dev endpoint is reachable only by a provider_dev credential issued
        # for that very endpoint, in that very provider. The audience check is
        # explicit: `AuthContextV2`'s validator refuses an endpoint scope on a
        # consumer or operator context, but `model_copy(update=)` and
        # `model_construct` skip that validator, so a consumer context can carry the
        # dev endpoint's ids and would pass both equalities below (F2P review B1).
