...
MUTANTS: tuple[Mutant, ...] = (
    ...
    _m("freeze_skipped", "the source admission is frozen before the drain is measured",
       T, "        await flag(ADMISSION_FLAG[source], False)\n", "",
    ...
    _m("freeze_only_still_enables", "a freeze-only run leaves both regimes paused",
       T, "        for name in () if freeze_only else ENABLE[target]:",
       "        for name in ENABLE[target]:",
       "test_credit_cutover__apply_freezes_first_drains_bounded_and_enables_last"),
    ...
    _m("straddler_wait_unbounded",
       ...
       "test_credit_cutover__apply_waits_out_every_transaction_open_at_the_freeze",
       dies_by=("RuntimeError",)),
    _m("flag_lock_unbounded",
       "every flag write waits for a locked flag (D10: admissions read it FOR SHARE) at most "
       "the drain's remaining bound (the PG timed straddle case is the database oracle)",
       T, "lock_timeout_s=max(deadline - monotonic(), 0.0))",
       "lock_timeout_s=float(\"inf\"))",
       "test_credit_cutover__a_flag_an_admission_holds_is_waited_for_within_the_bound"),
    _m("flag_lock_timeout_unmapped",
       "a flag write that runs out of its bound refuses with open_transactions and a report "
       "(declared: the raw FlagLocked escapes)",
    ...
    _m("flag_lock_unmapped",
       ...
       "test_flag__a_regime_flag_an_unknown_flag_and_a_locked_flag_change_nothing",
       dies_by=("FlagLocked",)),
    # --- G8-FLAG-2 (0-G8FLAG-R2/R3, 1-G8FLAG-R6) ---------------------------------------
    _m("flag_audits_as_transition", "a flag write is audited as operation `flag` (0-G8FLAG-R2)",
       T, "op._once(\"flag\", idempotency_key", "op._once(\"transition\", idempotency_key",
       "test_flag__signup_grant_goes_off_and_on_through_the_audited_writer_as_the_operator"),
    _m("flag_lock_bound_hardcoded", "the writer waits at most the operator's bound (0-G8FLAG-R3)",
       T, "                                           lock_timeout_s=lock_timeout_s)",
       "                                           lock_timeout_s=30.0)",
       "test_flag__signup_grant_goes_off_and_on_through_the_audited_writer_as_the_operator"),
    _m("flag_cli_lock_bound_constant", "--lock-timeout-s reaches the writer (0-G8FLAG-R3)",
       C, "lock_timeout_s=a.lock_timeout_s, **k)", "lock_timeout_s=5.0, **k)",
       "test_flag__signup_grant_goes_off_and_on_through_the_audited_writer_as_the_operator"),
    _m("flag_dry_run_requires_a_direction",
       "a dry run needs no --on/--off (1-G8FLAG-R6; declared: argparse exits)",
       C, "    switch = f.add_mutually_exclusive_group()",
       "    switch = f.add_mutually_exclusive_group(required=True)",
       "test_flag__the_dry_run_reads_without_a_key_and_writes_nothing", dies_by=("SystemExit",)),
