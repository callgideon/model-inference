...
U = "media/uploads.py"        # M3: owned uploads
...
MUTANTS: tuple[Mutant, ...] = (
    ...
    _m("stored_content_type_is_the_declared_one",
       ...
       "test_the_container_wins_over_the_declared_type"),
    # === M3: owned uploads (media/uploads.py) ===========================================
    _m("unknown_constraints_ignored", "a constraint nobody enforces is refused, not ignored",
       U, "unknown = set(constraints) - CONSTRAINTS", "unknown = set()",
       "test_create_refuses_constraints_the_caller_shapes"),
    _m("upload_cap_unbounded", "max_bytes is capped at MAX_MEDIA_BYTES",
       U, '                           "max_bytes", self.limits.max_media_bytes)',
       '                           "max_bytes", 2 ** 62)',
       "test_create_refuses_constraints_the_caller_shapes"),
    _m("byte_count_accepts_bool", "True is not a byte count",
       U, "if isinstance(value, bool) or not isinstance(value, int)",
       "if not isinstance(value, int)",
       "test_create_refuses_constraints_the_caller_shapes"),
    _m("declared_size_unvalidated", "a declared size is a byte count within max_bytes",
       U, '            size = _count(size, "bytes", max_bytes)', "            size = size",
       "test_create_refuses_constraints_the_caller_shapes"),
    _m("accepted_mime_not_allowlisted", "accepted types are a subset of the deployment's",
       U, "isinstance(mime, str) and mime in allowed for mime in mimes",
       "isinstance(mime, str) for mime in mimes",
       "test_create_refuses_constraints_the_caller_shapes"),
    _m("empty_accept_list_allowed", "an empty accept list is refused, not vacuously valid",
       U, "if not isinstance(mimes, (list, tuple)) or not mimes \\",
       "if not isinstance(mimes, (list, tuple)) \\",
       "test_create_refuses_constraints_the_caller_shapes"),
    _m("declared_digest_unvalidated", "a declared digest is sha256:<64 lowercase hex>",
       U, "            valid_digest(digest)", "            pass",
       "test_create_refuses_constraints_the_caller_shapes"),
    _m("duplicate_handle_accepted", "a repeated handle never takes over an existing upload",
       U, "if handle in self.uploads or not UPLOAD_HANDLE_RE.fullmatch(handle):",
       "if not UPLOAD_HANDLE_RE.fullmatch(handle):",
    ...
    _m("destination_carries_the_org", "R61(1): the destination is infrx-upload:upl_<id> only",
       U, "destination_ref=UPLOAD_REF_SCHEME + handle",
       'destination_ref=UPLOAD_REF_SCHEME + org_id + ":" + handle',
       "test_create_issues_an_opaque_handle_and_a_constrained_destination"),
    _m("upload_window_never_closes", "past expires_at an upload is upload_expired",
       U, "        if self.now() >= upload.expires_at:", "        if False:",
    ...
    _m("upload_window_excludes_its_end", "the window is closed at expires_at, not after it",
       U, "        if self.now() >= upload.expires_at:", "        if self.now() > upload.expires_at:",
       "test_an_expired_window_is_upload_expired_and_stays_closed"),
    _m("closed_upload_reopens", "a refused or completed upload takes no second attempt",
       U, "        if upload.state is not UploadState.created:",
       "        if upload.state is UploadState.expired:",
       "test_a_refused_upload_stays_refused", "test_a_completed_upload_accepts_no_more_bytes"),
    _m("put_uncapped", "put_upload refuses bytes over the upload's cap before storing them",
       U, "        if len(data) > upload.max_bytes:\n            raise errors.RequestTooLarge(",
       "        if False:\n            raise errors.RequestTooLarge(",
       "test_put_upload_is_bounded_and_write_once"),
    _m("upload_owner_unchecked", "another org's upload handle is not_found",
       U, "        if upload is None or upload.org_id != org_id:", "        if upload is None:",
       "test_another_org_cannot_put_finalize_or_resolve"),
    _m("finalize_unlocked", "concurrent finalizes verify and copy once",
       U, "        async with upload.lock:\n            return await self._finalize(upload)",
       "        return await self._finalize(upload)",
    ...
    _m("refinalize_ignores_a_changed_destination",
       ...
       U, "if await self.objects.head(key) not in (None, upload.ref.digest):", "if False:",
    ...
    _m("finalize_not_idempotent", "a retry of a completed upload returns the same ref",
       U, "with other bytes\")\n            return upload.ref", "with other bytes\")",
       "test_a_retry_returns_the_same_completed_handle"),
    _m("head_size_unchecked", "an oversize object is refused before it is downloaded",
       U, "        if size > upload.max_bytes:", "        if False:",
       "test_an_oversize_object_is_refused_without_being_downloaded"),
    _m("byte_size_unchecked", "the bytes, not the HEAD, are the size authority",
       U, "        if len(data) > upload.max_bytes:\n            raise self._abort(",
       "        if False:\n            raise self._abort(",
       "test_a_head_that_understates_the_size_is_caught_by_the_bytes"),
    _m("declared_size_unverified", "a declared exact size is checked against the bytes",
       U, "if upload.size is not None and len(data) != upload.size:", "if False:",
       "test_a_declared_size_is_verified"),
    _m("declared_digest_unverified", "a declared digest is checked against the bytes",
       U, "if upload.digest is not None and digest != upload.digest:", "if False:",
       "test_a_declared_digest_is_verified"),
    _m("declared_digest_prefix_compared", "the whole declared digest is compared (review)",
       U, "if upload.digest is not None and digest != upload.digest:",
       "if upload.digest is not None and digest[:20] != upload.digest[:20]:",
       "test_a_declared_digest_is_verified"),
    _m("use_time_recheck_case_folded", "the use-time re-check is exact, not case-folded (review)",
       ...
       "and (await self.objects.head(media.storage_ref) or \"\").lower() != media.digest:",
    ...
    _m("probe_refusal_not_final", "bytes the probe refuses abort the upload",
       U, "            raise self._abort(upload, refused)", "            raise refused",
       "test_bytes_the_probe_refuses_abort_the_upload"),
    _m("accepted_type_unchecked", "a type outside the upload's accept list is refused",
       U, "        if mime not in upload.accepted_mime:", "        if False:",
       "test_an_unaccepted_type_is_refused"),
    _m("finalize_replaces_named_content", "completing never replaces what the handle names",
       U, "if existing is not None and existing.digest != digest:", "if False:",
       "test_finalizing_never_replaces_what_the_handle_already_names"),
    _m("refusal_not_recorded", "an aborted upload stays aborted",
       U, "        upload.state = UploadState.aborted\n        return error",
       "        return error",
       "test_a_refused_upload_stays_refused"),
    _m("upload_not_at_the_source_key",
       "a finalized upload lives at the content-addressed source key prepare rebuilds",
       U, '                                             "source"),',
       '                                             "upload"),',
       "test_finalize_measures_the_bytes_that_arrived",
       "test_an_uploaded_clip_is_prepared_like_any_source"),
    _m("unfinalized_upload_resolves", "an upload resolves only once finalized",
       U, "        if upload is not None and upload.state is not UploadState.finalized:",
       "        if False:",
       "test_resolve_refuses_an_upload_that_is_not_finalized_even_if_its_handle_is_indexed"),
    _m("upload_state_answered_across_orgs", "an upload's state is answered only to its org",
       U, "        if upload is not None and upload.org_id == org_id \\\n",
    ...
    _m("changed_upload_object_resolves", "a finalized upload's object is re-checked at use",
       U, "and await self.objects.head(media.storage_ref) != media.digest:", "and False:",
    ...
    _m("collected_upload_still_finalizes", "a collected upload's record goes with it",
    ...
    _m("lapsed_window_left_open", "a pass closes a lapsed upload window",
       G, "if upload.state is UploadState.created and now >= upload.expires_at:", "if False:",
    ...
    _m("refused_record_kept_forever", "a refused upload record is dropped after the grace",
       G, "                del store.uploads[handle]      # a refused", "                pass  # a refused",
