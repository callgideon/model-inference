...
MUTANTS: tuple[Mutant, ...] = (
    ...
    _m("dedupe_on_the_raw_request_id", "duplicates are detected on the canonical id",
       S, "        key = canonical_id(candidate.request_id)", "        key = candidate.request_id",
       "test_deduplication_is_on_the_canonical_request_id"),
    _m("a_respelled_duplicate_is_a_conflict",
       "two rows differing only in how the id was spelled are one trace, not a conflict",
       S, "        elif all(_same_facts(row, rows[0]) for row in rows[1:]):",
       "        elif all(row == rows[0] for row in rows[1:]):",
       "test_deduplication_is_on_the_canonical_request_id"),
    _m("canonical_id_keeps_the_case", "the canonical id is trimmed and lower-case",
       S, "    return request_id.strip().lower() if isinstance(request_id, str) else repr(request_id)",
       "    return request_id if isinstance(request_id, str) else repr(request_id)",
       "test_deduplication_is_on_the_canonical_request_id"),
