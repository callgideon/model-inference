...
MUTANTS: tuple[Mutant, ...] = (
    ...
    _m("consent_window_ignored", "consent is not retroactive (R56)",
       S, "    if not within_consent_window(candidate.started_at, consent):",
       "    if False:",
       "test_evaluation_consent_is_not_retroactive"),
    _m("consent_window_start_ignored", "a trace older than the consent is not eligible",
       S, "    if started_at < consent.effective_at:\n        return False",
       "    if False:\n        return False",
       "test_evaluation_consent_is_not_retroactive"),
    _m("consent_window_end_ignored", "a trace after the revocation instant is not eligible",
       S, "    return consent.revoked_at is None or started_at < consent.revoked_at",
       "    return True",
       "test_the_consent_window_is_half_open_at_both_ends"),
    ...
    _m("duplicates_sampled_twice", "one row per request_id before stratifying (R56)",
       S, "    unique, duplicates = deduplicate(tuple(owned))",
       "    unique, duplicates = list(owned), []",
    ...
    _m("boolean_score_accepted", "True is not 1 and 4.0 is not 4",
       R, "        if type(score) is not int:", "        if not isinstance(score, int):",
    ...
    _m("non_string_key_reaches_the_key_arithmetic",
       ...
       R, "    unnamed = [key for key in payload if type(key) is not str]", "    unnamed = []",
       "test_a_hostile_payload_is_rejected_rather_than_raised", dies_by=("TypeError",)),
    # --- R2-B1: the parse guard, unstorable text and exact types ---------------------
    _m("recursion_error_escapes_the_parse_guard",
       "a deeply nested payload is rejected, not raised (R2-B1)",
       R, "    except RecursionError:", "    except DuplicateKey:",
       "test_a_deeply_nested_payload_is_rejected_rather_than_raised",
       dies_by=("RecursionError",)),
    _m("lone_surrogate_accepted", "text that cannot be encoded is refused at the boundary",
       R, '        value.encode("utf-8")', "        pass",
       "test_a_lone_surrogate_is_refused_at_the_boundary"),
    _m("payload_type_checked_loosely", "a hostile dict subclass is refused at the door",
       R, "    if type(payload) is not dict:", "    if not isinstance(payload, dict):",
       "test_a_hostile_subclass_cannot_make_the_validator_raise",
       dies_by=("RuntimeError",)),
    _m("score_type_checked_loosely", "a hostile int subclass is not a score",
       R, "        if type(score) is not int:",
       "        if isinstance(score, bool) or not isinstance(score, int):",
       "test_a_hostile_subclass_cannot_make_the_validator_raise",
       dies_by=("RuntimeError",)),
    _m("rationale_type_checked_loosely", "a hostile str subclass is not a rationale",
       R, "        if type(rationale) is not str or not rationale.strip():",
       "        if not isinstance(rationale, str) or not rationale.strip():",
       "test_a_hostile_subclass_cannot_make_the_validator_raise",
       dies_by=("RuntimeError",)),
    _m("describe_lets_an_undescribable_value_raise",
       "an untrusted value is described without raising, even by its type name",
       R, '        return "<undescribable>"', "        raise",
       "test_describe_reports_what_a_value_is_never_what_it_says", dies_by=("RuntimeError",)),
    ...
    # --- R2-B3: a detail says what a value is, never what it says ---------------------
    _m("detail_cap_overshoots", "a detail is at most MAX_DETAIL_CHARS in total (R2-B3)",
       R, '                               self.detail[:MAX_DETAIL_CHARS - 3] + "...")',
       '                               self.detail[:MAX_DETAIL_CHARS] + "...")',
       "test_every_rejection_detail_is_bounded",
       "test_a_hostile_payload_is_rejected_rather_than_raised"),
    _m("describe_echoes_a_string", "a string is reported by length, never quoted (R2-B3)",
       R, '            return f"str of {len(value)} characters"',
       "            return repr(value)",
       "test_describe_reports_what_a_value_is_never_what_it_says",
       "test_no_rejection_ever_echoes_the_payload"),
    _m("describe_echoes_an_integer", "an out-of-range score is reported by sign and digits",
       R, '            return f"{\'negative\' if value < 0 else \'positive\'} int of {digits} digits"',
       "            return str(value)",
       "test_describe_reports_what_a_value_is_never_what_it_says",
       "test_no_rejection_ever_echoes_the_payload"),
    _m("unexpected_key_echoed", "an unexpected key is counted and typed, never echoed",
       R, '        return reject("unexpected_field",\n'
          '                      f"{len(unexpected)} unexpected key(s), the first a "\n'
          '                      f"{describe(unexpected[0])}")',
       '        return reject("unexpected_field", f"unexpected {unexpected[0]}")',
       "test_a_missing_or_extra_field_is_rejected_by_name",
       "test_no_rejection_ever_echoes_the_payload"),
    _m("duplicate_key_echoed", "a duplicate key is typed, never echoed",
       R, '            raise DuplicateKey(f"a key appears twice ({describe(key)})")',
       '            raise DuplicateKey(f"duplicate key {key}")',
       "test_no_rejection_ever_echoes_the_payload"),
    _m("json_error_echoes_the_document", "a parse error names the fault and offset only",
       R, '        return reject("malformed_json", f"{exc.msg} at position {exc.pos}")',
       '        return reject("malformed_json", str(exc.doc))',
       "test_no_rejection_ever_echoes_the_payload"),
    ...
    _m("ledger_accepts_any_sample", "a ledger is bounded by the run's planned samples",
       R, "        if not planned:", "        if False:",
       "test_a_ledger_belongs_to_one_run_and_its_planned_samples"),
    _m("ledger_raises_on_an_unusable_sample_id",
       "an unhashable sample id is a typed refusal, not a TypeError",
       R, "        try:\n            planned = result.sample_id in self.sample_ids\n"
          "        except Exception:                              # noqa: BLE001 - see the comment",
       "        if True:\n            planned = result.sample_id in self.sample_ids\n"
       "        if False:",
       "test_a_ledger_refuses_an_unusable_sample_id_rather_than_raising",
       dies_by=("TypeError",)),
    # --- R2 small: the tenant filter, the canonical id and malformed rows -------------
    _m("dedupe_runs_before_the_tenant_filter",
       "a foreign row cannot exclude my trace as a duplicate",
       S, "        elif candidate.org_id != org_id:", "        elif False:",
       "test_another_orgs_row_cannot_exclude_my_trace_as_a_duplicate",
       "test_another_tenants_trace_is_never_a_sample",
       "test_a_row_with_no_organization_is_not_mine"),
    # --- R3-B1: one spelling of a request id, validated, never echoed ------------------
    _m("any_nonblank_string_is_a_request_id",
       "a request id is the frozen lower-case UUIDv4 form (R3-B1)",
       S, "    if not ids.is_request_id(candidate.request_id):",
       "    if not (isinstance(candidate.request_id, str) and candidate.request_id.strip()):",
       "test_an_id_that_is_not_the_frozen_request_id_form_is_malformed",
       "test_a_respelled_row_can_never_be_sampled_twice",
       "test_the_draw_and_the_ids_it_emits_are_independent_of_the_scan_order",
       "test_an_invalid_id_is_described_in_the_exclusion_not_echoed"),
    _m("an_invalid_id_is_echoed_into_the_exclusion",
       "an invalid request id is described, never echoed (R3-B1)",
       S, '    return f"<invalid request_id: {describe(request_id)}>"',
       "    return str(request_id)",
       "test_an_invalid_id_is_described_in_the_exclusion_not_echoed"),
    _m("a_malformed_row_is_interpreted",
       "one bad row is excluded, never interpreted and never fatal",
       S, "        elif _malformed(candidate, now):", "        elif False:",
       "test_one_bad_row_is_excluded_and_never_interpreted",
       dies_by=("TypeError", "KeyError", "AttributeError")),
    _m("a_future_row_is_a_candidate", "a trace stamped after `now` is not a candidate",
       S, "    return started > now", "    return False",
       "test_one_bad_row_is_excluded_and_never_interpreted",
       "test_evaluation_consent_is_not_retroactive"),
    ...
    _m("dry_run_can_submit", "only the exact mode live authorizes a submission",
       C, '        raise errors.BudgetExceeded(f"judge mode {mode!r} is not {JUDGE_MODE_LIVE!r} "\n'
    ...
    _m("zero_budget_can_submit", "a live submission needs a positive budget",
       C, '    budget = _positive_money(getattr(settings, "judge_live_budget_usd", None))',
       '    budget = _positive_money(getattr(settings, "judge_live_budget_usd", None)) or money.MAX_VALUE',
       "test_the_defaults_can_never_authorize_a_submission",
    ...
    _m("priced_from_per_sample_only", "a half-filled estimate is not a priced one",
       ...
       "                and True)",
       "test_the_guard_recomputes_the_total_and_never_trusts_the_estimate"),
    # --- R2-B4: the five clauses whose mutants survived round 2 -----------------------
    _m("superseded_price_version_accepted",
       "the estimate's price version must still be the effective one (R2-B4)",
       C, "    if estimate.price_version is not None and estimate.price_version != rate.price_version:",
       "    if False:",
       "test_an_estimate_priced_at_a_superseded_version_is_refused_even_at_the_same_amount"),
    _m("priced_without_a_version",
       "an estimate with no price version authorizes nothing (R2-B4)",
       C, "        return (isinstance(self.price_version, str) and bool(self.price_version.strip())",
       "        return (True",
       "test_an_estimate_with_no_price_version_is_not_priced"),
    _m("consent_window_end_is_inclusive",
       "a trace **at** the revocation instant is outside the window (R2-B4)",
       S, "    return consent.revoked_at is None or started_at < consent.revoked_at",
       "    return consent.revoked_at is None or started_at <= consent.revoked_at",
       "test_the_consent_window_is_half_open_at_both_ends"),
    _m("consent_window_start_is_exclusive",
       "a trace **at** effective_at is inside the window",
       S, "    if started_at < consent.effective_at:", "    if started_at <= consent.effective_at:",
       "test_the_consent_window_is_half_open_at_both_ends",
       "test_evaluation_consent_is_not_retroactive"),
    _m("the_plan_prices_at_the_lookback_start",
       "the plan prices at `now`, not at `since` (R2-B4)",
       D, "                                   ceilings=ceilings, at=now)",
       "                                   ceilings=ceilings, at=since)",
       "test_the_plan_prices_at_now_not_at_the_lookback_start"),
    _m("the_guard_is_asked_at_the_lookback_start",
       "the authorization is decided at the same instant as the estimate (R2-B4)",
       D, "                                                                     rates=rates, at=now))",
       "                                                                     rates=rates, at=since))",
       "test_the_plan_prices_at_now_not_at_the_lookback_start"),
    ...
    _m("worst_case_escapes_the_money_domain",
       ...
       C, "        return money.parse(money.ceiling(product))",
       "        return money.ceiling(product)",
       "test_an_unrepresentable_worst_case_refuses_rather_than_raising_arithmetic"),
    _m("worst_case_uses_the_ambient_context",
       "the worst-case product runs in money.arithmetic_context(), never the caller's (R2-B2)",
       C, "        product = money.arithmetic_context().multiply(per_sample, Decimal(samples))",
       "        product = per_sample * samples",
       "test_the_worst_case_ignores_the_ambient_decimal_context"),
    _m("worst_case_takes_any_ceilings", "a worst case needs validated token ceilings",
       C, "    if not isinstance(ceilings, TokenCeilings):", "    if False:",
       "test_an_unrepresentable_worst_case_refuses_rather_than_raising_arithmetic",
    ...
    # --- R3-B2: four clauses whose mutants survived round 3 -----------------------------
    _m("guard_mode_getattr_default_live",
       "a settings object with no judge_mode never authorizes (fail closed, R3-B2)",
       C, '    mode = getattr(settings, "judge_mode", None)',
       '    mode = getattr(settings, "judge_mode", JUDGE_MODE_LIVE)',
       "test_a_settings_object_with_no_mode_never_authorizes"),
    _m("priced_blank_version_ok", "a blank price version is not a version (R3-B2)",
       C, "bool(self.price_version.strip())", "bool(self.price_version)",
       "test_a_blank_price_version_is_not_a_version"),
    _m("b1_entry_isinstance", "a criterion entry is exact-typed (R3-B2)",
       R, "        if type(entry) is not dict or set(entry) != {\"score\", \"rationale\"}:",
       '        if not isinstance(entry, dict) or set(entry) != {"score", "rationale"}:',
       "test_a_hostile_subclass_cannot_make_the_validator_raise",
       dies_by=("RuntimeError",)),
    _m("b1_key_isinstance", "a payload key is exact-typed (R3-B2)",
       R, "    unnamed = [key for key in payload if type(key) is not str]",
       "    unnamed = [key for key in payload if not isinstance(key, str)]",
       "test_a_hostile_subclass_cannot_make_the_validator_raise",
       dies_by=("RuntimeError",)),
    # --- R3 small: source shape, status range, feedback elements, the report field -----
    _m("source_answer_unchecked", "a source that answers a non-iterable is a typed refusal",
       D, "    if isinstance(rows, (str, bytes)) or not isinstance(rows, Iterable):",
       "    if False:",
       "test_a_source_that_does_not_answer_an_iterable_is_refused",
       dies_by=("TypeError", "AttributeError")),
    _m("source_awaitable_unchecked", "a source must be an async function",
       D, "    if not isinstance(pending, Awaitable):", "    if False:",
       "test_an_async_generator_source_is_refused", dies_by=("TypeError",)),
    _m("plan_mode_is_a_literal", "the plan reports the mode it ran under",
       D, "mode=settings.judge_mode,", 'mode="dry_run",',
       "test_the_plan_reports_the_mode_it_ran_under"),
    _m("any_http_status_accepted", "a status outside 100..599 is malformed",
       S, "    if not MIN_HTTP_STATUS <= candidate.http_status <= MAX_HTTP_STATUS:",
       "    if False:",
       "test_one_bad_row_is_excluded_and_never_interpreted"),
    _m("feedback_elements_unchecked", "every feedback element is a Feedback row",
       S, "    if not all(isinstance(entry, Feedback) for entry in candidate.feedback):",
       "    if False:",
       "test_one_bad_row_is_excluded_and_never_interpreted", dies_by=("AttributeError",)),
    _m("row_type_unchecked", "a candidate is a TraceCandidate, checked before its org",
       S, "        if not isinstance(candidate, TraceCandidate):", "        if False:",
       "test_a_row_that_is_not_a_candidate_at_all_is_excluded", dies_by=("AttributeError",)),
    _m("ledger_hash_only_catches_typeerror",
       "a sample id whose hashing raises anything is a typed refusal",
       R, "        except Exception:                              # noqa: BLE001 - see the comment",
       "        except TypeError:",
       "test_a_ledger_refuses_an_unusable_sample_id_rather_than_raising",
       dies_by=("RuntimeError",)),
