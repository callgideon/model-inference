...
MUTANTS: tuple[Mutant, ...] = (
    ...
    _m("lc_constraints_accept_extra_names", "a create body names no tenant, handle or expiry",
       LC, '    """What a create body may fix, and nothing else: no tenant, handle, window or path."""\n',
       '    """What a create body may fix, and nothing else: no tenant, handle, window or path."""\n'
       '    model_config = RecordV2.model_config | {"extra": "ignore"}\n',
       "upload_restart__the_tenant_handle_and_window_are_the_stores",
    ...
    _m("lc_payload_unprotected_by_its_job", "a payload is referenced by its admitted job",
       LCF, "        if identity.job_id is not None and self._job_live(identity.job_id, now):",
