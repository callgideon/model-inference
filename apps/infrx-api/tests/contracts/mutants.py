...
MUTANTS: tuple[Mutant, ...] = (
    ...
    _m("credit_second_cancel_rewrites_the_cause", "a second CREDIT cancel never rewrites the committed cause",
       ...
       "            if job.terminal:\n                # Completion won the race; a completed job stays completed.\n                if job.state is JobState.cancelled:\n                    job.outcome = job.outcome.model_copy(update={\"cause\": cause})\n                return job.outcome",
       "credit_settle__cancel_records_its_cause_and_settles_by_r21"),
    # --- F2C.a lifecycle: uploads, execution readiness, content cleanup -------------------
    # Several of these ARE the pre-F2C behaviour, reintroduced: per-process ticket state
    # (RV-02), no marker for an empty attach (RV-05), liveness without durable references
    # (RV-03). Killed in tests/contracts/v2/test_lifecycle.py.
    _m("lc_reopen_loses_the_tickets", "every upload step survives a new process (RV-02)",
       LCF, "        return FakeLifecycle(self.jobs, self.clock, self.ids, durable=self.d,",
       "        return FakeLifecycle(self.jobs, self.clock, self.ids, durable=None,",
       "upload_restart__every_step_survives_a_new_process"),
    _m("lc_foreign_handle_answers", "another tenant's handle is not_found",
       LCF, "        if ticket is None or ticket.org_id != org_id:", "        if ticket is None:",
       "upload_restart__the_tenant_handle_and_window_are_the_stores"),
    _m("lc_constraints_accept_extra_names", "a create body names no tenant, handle, expiry or version",
       LC, "        if set(body) - CONSTRAINT_NAMES:", "        if False:",
       "upload_restart__the_tenant_handle_and_window_are_the_stores",
       "test_constraints_default_to_the_deployment_and_refuse_everything_else"),
    _m("lc_put_accepts_other_bytes", "one handle names one set of bytes",
       LCF, "                if (ticket.received.bytes, ticket.received.digest) != (bytes, digest):",
       "                if False:", "upload_restart__one_handle_names_one_set_of_bytes"),
    _m("lc_open_at_expiry", "a put or completion at expires_at exactly is upload_expired",
       LCF, '        if self.clock.now() >= ticket.expires_at:\n            raise refuse(R.upload_expired, "the upload window closed")',
       '        if self.clock.now() > ticket.expires_at:\n            raise refuse(R.upload_expired, "the upload window closed")',
       # The port answers upload_expired first; without it the record's own window rule
       # (a receipt strictly before expires_at) refuses the write - a second guard, declared.
       "upload_restart__the_window_bounds_completion_and_use", dies_by=("ValidationError",)),
    _m("lc_resolve_ignores_the_window", "a finalized upload is unusable from expires_at on (R99(b))",
    ...
    _m("lc_expire_repeats", "expire closes an open ticket once",
       LCF, "                          if t.state is UploadState.created and now >= t.expires_at),",
       "                          if now >= t.expires_at),",
       "upload_restart__the_window_bounds_completion_and_use", dies_by=("ValidationError",)),
    _m("lc_failed_check_not_final", "a failed constraint aborts the ticket in the same transaction",
       LCF, "                self._put(ticket, state=UploadState.aborted, refusal=failed)",
       "                pass", "upload_restart__a_failed_check_is_final"),
    _m("lc_empty_attach_writes_no_marker", "a text job is ready with an EMPTY manifest (RV-05)",
       LCF, "            self.d.readiness[admission.request_id] = readiness",
       "            if sources:\n                self.d.readiness[admission.request_id] = readiness",
       "admission_ready__a_text_job_is_ready_with_an_empty_manifest"),
    _m("lc_missing_marker_reads_as_empty", "no marker is not_ready, never an empty manifest (RV-05)",
       LCF, "        return self.d.readiness.get(job_id) if ids.is_request_id(job_id) else None",
       "        return self.d.readiness.get(job_id, ExecutionReadiness(\n"
       "            job_id=job_id, org_id=job_id, sources=(), ready_at=self.clock.now()))",
       "admission_ready__a_job_with_no_marker_is_never_claimable"),
    _m("lc_claim_without_marker", "preparation never claims a job without its marker",
       LCF, "        if job_id not in self.d.readiness:", "        if False:",
       "admission_ready__a_job_with_no_marker_is_never_claimable"),
    _m("lc_manifest_ignores_the_digest", "a manifest ref carries its content row's digest",
       LCF, "                    or row.identity.digest != ref.digest or ref.org_id != request.org_id \\",
       "                    or ref.org_id != request.org_id \\",
       "admission_ready__the_manifest_is_the_orgs_own_live_content"),
    _m("lc_manifest_ignores_the_tenant", "a manifest ref is the request organization's content",
       LCF, "            if row is None or row.identity.org_id != request.org_id \\\n"
            "                    or row.identity.kind is not ContentKind.source \\\n"
            "                    or row.identity.digest != ref.digest or ref.org_id != request.org_id \\",
       "            if row is None \\\n"
       "                    or row.identity.kind is not ContentKind.source \\\n"
       "                    or row.identity.digest != ref.digest \\",
       "admission_ready__the_manifest_is_the_orgs_own_live_content"),
    _m("lc_expectation_ignored", "the pinned card must be the runtime's (R69, in the transaction)",
       LCF, "                if pins.rate_card_version != expectation.rate_card_version:",
       "                if False:",
    ...
    _m("lc_manifest_skips_the_ticket", "an upload ref is admitted only through its usable ticket",
       LCF, "                ticket = self._resolve(request.org_id, ref.handle)",
       "                ticket = self.d.tickets[ref.handle]",
       "admission_ready__an_expired_upload_cannot_be_admitted"),
    _m("lc_manifest_reference_ignored", "a job's manifest reference protects its source (RV-03)",
       LCF, "        if any(ref.generation == row.generation and self._job_live(ref.job_id, now)",
       "        if False and any(ref.generation == row.generation and self._job_live(ref.job_id, now)",
       "retention_durable__a_referenced_object_is_neither_a_candidate_nor_claimable"),
    _m("lc_terminal_job_releases_at_once", "a terminal job's reference lasts until retain_until",
       LCF, "        return now < until", "        return False",
       "retention_durable__a_referenced_object_is_neither_a_candidate_nor_claimable"),
    _m("lc_tombstone_without_recheck", "the tombstone rechecks references under the lock",
       LCF, "                self._recheck(row, now)\n                row = self._save(",
       "                row = self._save(",
       "retention_durable__admission_wins_over_a_claim_that_has_not_tombstoned"),
    _m("lc_register_over_a_tombstone", "a tombstoned key waits for its delete acknowledgement",
       LCF, '        if row.state is LifecycleState.tombstoned:\n            raise refuse(R.content_retiring, "a delete',
       '        if False:\n            raise refuse(R.content_retiring, "a delete',
       "retention_durable__a_tombstone_refuses_new_use_until_the_delete_is_acked"),
    _m("lc_admit_a_tombstoned_source", "admission refuses a source whose delete is pending",
       LCF, "            if row.state is LifecycleState.tombstoned:\n                raise refuse(R.content_retiring, f\"source",
       "            if False:\n                raise refuse(R.content_retiring, f\"source",
       "retention_durable__a_tombstone_refuses_new_use_until_the_delete_is_acked"),
    _m("lc_delayed_ack_retires_the_new_generation", "an old tombstone never retires a recreated key",
       LCF, "            if row.generation != tombstone.generation or row.state is LifecycleState.deleted:",
       "            if row.state is LifecycleState.deleted:",
       "retention_durable__a_tombstone_refuses_new_use_until_the_delete_is_acked"),
    _m("lc_claim_ignores_a_live_claim", "one unexpired claim at a time",
       LCF, "            if row.claim is not None and now < row.claim.expires_at:",
       "            if False:", "retention_durable__claims_are_leased_and_fenced"),
    _m("lc_tombstone_ignores_the_fence", "a lapsed or superseded claim cannot tombstone",
    ...
    _m("lc_finalized_ticket_does_not_protect_its_source", "a usable ticket references its source",
       LCF, "        return any(t.finalized is not None and t.finalized.content_id == row.content_id",
       "        return False and any(t.finalized is not None and t.finalized.content_id == row.content_id",
       "retention_durable__an_upload_protects_its_destination_and_source"),
    _m("lc_payload_unprotected_by_its_job", "a payload is referenced by its admitted job",
       LCF, "        if identity.job_id is not None and self._job_live(identity.job_id, now, identity.kind):",
       "        if False:", "retention_durable__a_payload_is_protected_by_its_job_once_admitted"),
    _m("lc_cursor_repeats_its_row", "each candidate appears once across pages",
       LCF, "                rows = [r for r in rows if (r.eligible_at, r.content_id) > start]",
       "                rows = [r for r in rows if (r.eligible_at, r.content_id) >= start]",
       "retention_durable__candidates_page_without_loss_or_repeat"),
    _m("lc_empty_manifest_is_the_default", "sources is REQUIRED: absent is never []",
       LC, "    sources: tuple[ManifestSource, ...]\n    ready_at: Timestamp",
       "    sources: tuple[ManifestSource, ...] = ()\n    ready_at: Timestamp",
       "test_an_empty_manifest_is_a_completed_fact_and_never_the_default",
       "test_a_missing_required_or_an_unexpected_field_fails_deterministically"),
    _m("lc_view_ready_without_a_count", "ready carries a count and an instant; not_ready neither",
       LC, "        if ready != (self.source_count is not None) or ready != (self.ready_at is not None):",
       "        if False:", "test_an_empty_manifest_is_a_completed_fact_and_never_the_default"),
    ...
    # --- F2C slice-a verifier findings (F1-F10), folded in slice b -----------------------
    _m("lc_legacy_admission_writes_no_marker", "admit_ready writes the marker in BOTH regimes (F1)",
       LCF, "            self.d.readiness[admission.request_id] = readiness",
       "            if credit:\n                self.d.readiness[admission.request_id] = readiness",
       "admission_ready__a_text_job_is_ready_with_an_empty_manifest"),
    _m("lc_abort_not_persisted", "an abort is durable (F2)",
       LCF, "            return self._put(ticket, state=UploadState.aborted, refusal=R(refusal))",
       "            return UploadTicket.model_validate(\n"
       "                {**ticket.model_dump(), \"state\": UploadState.aborted, \"refusal\": R(refusal)})",
       "upload_restart__oversize_puts_and_aborts_are_final_everywhere"),
    _m("lc_abort_not_idempotent", "an aborted ticket answers a repeat abort as it stands (F2)",
       LCF, "            if ticket.state is UploadState.aborted:\n                return ticket",
       "            if False:\n                return ticket",
       "upload_restart__oversize_puts_and_aborts_are_final_everywhere"),
    _m("lc_too_large_unchecked", "a put over max_bytes is too_large (F2)",
       LCF, "            if bytes > ticket.constraints.max_bytes:\n                raise refuse(R.too_large",
       "            if False:\n                raise refuse(R.too_large",
       # the receipt record's own cap is the second guard; the port answers too_large first
       "upload_restart__oversize_puts_and_aborts_are_final_everywhere", dies_by=("ValidationError",)),
    _m("lc_too_large_untyped", "too_large carries its reason, not a bare 400 (F2)",
       LCF, "                raise refuse(R.too_large, \"over the ticket's max_bytes\")",
       "                raise errors.InvalidRequest(\"over the ticket's max_bytes\")",
       "upload_restart__oversize_puts_and_aborts_are_final_everywhere"),
    _m("lc_abort_accepts_internal_reasons", "a ticket records only a public upload refusal (F8)",
       LCF, "        if refusal not in UPLOAD_ABORT_REASONS:\n            raise errors.InvalidRequest(",
       "        if False:\n            raise errors.InvalidRequest(",
       # the ticket record refuses it too; the port's typed refusal comes first
       "upload_restart__oversize_puts_and_aborts_are_final_everywhere", dies_by=("ValidationError",)),
    _m("lc_ticket_records_any_refusal", "the browser-safe ticket carries no internal refusal (F8)",
       LC, "        if self.refusal is not None and self.refusal not in UPLOAD_ABORT_REASONS:",
       "        if False:", "test_an_upload_ticket_is_one_fact"),
    _m("lc_duration_lax", "a measured duration is a strict finite number (F10)",
       LC, "    duration_s: StrictFloat = Field(ge=0, allow_inf_nan=False)",
       "    duration_s: float = Field(ge=0)", "test_an_upload_ticket_is_one_fact"),
    _m("lc_tombstoned_never_offered_again", "an unfinished delete is a candidate once its claim lapses (F3)",
       LCF, "        if row.state is LifecycleState.tombstoned:\n            return claim_free",
       "        if row.state is LifecycleState.tombstoned:\n            return False",
       "retention_durable__an_unfinished_delete_is_reconciled_by_a_higher_fence"),
    _m("lc_tombstoned_never_reclaimed", "a tombstoned row may be re-claimed to finish its delete (F3)",
       LCF, "            if row.generation != generation or row.state is LifecycleState.deleted:",
       "            if row.generation != generation or row.state is not LifecycleState.live:",
       "retention_durable__an_unfinished_delete_is_reconciled_by_a_higher_fence"),
    _m("lc_claim_inside_the_grace", "no claim before the persisted eligibility (F4)",
       LCF, "        if now < row.eligible_at:\n            raise refuse(R.not_eligible",
       "        if False:\n            raise refuse(R.not_eligible",
       "retention_durable__claims_are_leased_and_fenced"),
    _m("lc_candidates_offer_claimed_rows", "a row under an unexpired claim is not offered (F4)",
       LCF, "        claim_free = row.claim is None or now >= row.claim.expires_at",
       "        claim_free = True",
       "retention_durable__claims_are_leased_and_fenced",
       "retention_durable__an_unfinished_delete_is_reconciled_by_a_higher_fence"),
    _m("lc_page_unbounded", "a page holds at most MAX_PAGE rows (F4)",
       LCF, "        limit = min(limit, MAX_PAGE)", "        limit = limit",
       "retention_durable__candidates_page_without_loss_or_repeat"),
    _m("lc_repeat_ack_refused", "a repeated acknowledgement answers the deleted row (F4)",
       LCF, "            if row.generation != tombstone.generation or row.state is LifecycleState.deleted:",
       "            if row.generation != tombstone.generation:",
       "retention_durable__a_tombstone_refuses_new_use_until_the_delete_is_acked"),
    _m("lc_expire_ignores_its_limit", "expire closes at most `limit` tickets (F4)",
       LCF, "                         key=lambda t: (t.expires_at, t.upload_handle))[:limit]",
    ...
    _m("lc_manifest_any_kind", "only source content is a manifest source (F5)",
       ...
       "", "admission_ready__the_manifest_is_the_orgs_own_live_content"),
    _m("lc_manifest_ticket_other_content", "an upload ref resolves through ITS ticket's content (F5)",
       LCF, "                if ticket.finalized.content_id != row.content_id:", "                if False:",
       "admission_ready__the_manifest_is_the_orgs_own_live_content"),
    _m("lc_manifest_admits_deleted", "a deleted row is never admitted (F5)",
       LCF, "                    or row.state is LifecycleState.deleted:",
       "                    or False:",
       "retention_durable__a_tombstone_refuses_new_use_until_the_delete_is_acked"),
    _m("lc_register_foreign_key", "a key names one tenant (F6)",
       LCF, "        if row.identity.org_id != identity.org_id:\n            raise refuse(R.not_found",
       "        if False:\n            raise refuse(R.not_found",
       "retention_durable__eligibility_survives_restart_and_discovery"),
    _m("lc_register_other_bytes", "a live key names one set of bytes (F6)",
       LCF, "            raise refuse(R.bytes_changed, \"the key already names other bytes\")",
       "            pass", "retention_durable__eligibility_survives_restart_and_discovery"),
    _m("lc_put_retry_rewrites_the_receipt", "a put retry never rewrites when the bytes arrived (F7)",
       LCF, "                return ticket\n            return self._put(ticket, received=receipt)",
       "                return self._put(ticket, received=receipt)\n            return self._put(ticket, received=receipt)",
       "upload_restart__one_handle_names_one_set_of_bytes"),
    _m("lc_put_retry_ignores_the_size", "a put retry compares bytes as well as digest (F7)",
       LCF, "                if (ticket.received.bytes, ticket.received.digest) != (bytes, digest):",
       "                if ticket.received.digest != digest:",
       "upload_restart__one_handle_names_one_set_of_bytes"),
    _m("lc_completion_retry_ignores_mime_and_size", "a completion retry is the same source (F7)",
       LCF, "                if (done.digest, done.bytes, done.mime) != (source.digest, source.bytes,\n"
            "                                                           source.mime) \\",
       "                if (done.digest,) != (source.digest,) \\",
       "upload_restart__one_handle_names_one_set_of_bytes"),
    _m("lc_malformed_digest_untyped", "a malformed digest is a typed invalid_request (F7)",
       LCF, "            if not isinstance(digest, str) or not SHA256.fullmatch(digest):",
       "            if False:",
