...
MUTANTS: tuple[Mutant, ...] = (
    ...
    # No mutant for the arrival tag `_Flow(tag=self._virtual_time)`: it is provably
    # equivalent to `0.0` under the dispatch clamp (see `_flow`'s docstring and the Q1
    # evidence), so a mutant of it survives by construction rather than for want of a
    # case. The invariant the idle-credit case claims is killed by
    # `the_dispatch_start_is_not_clamped_to_the_virtual_time` and by
