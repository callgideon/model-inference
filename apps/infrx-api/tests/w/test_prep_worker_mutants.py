#!/usr/bin/env python3
"""R32/R40/R83: every invariant the preparation loop claims is killable by a named case.

    uv run --frozen pytest -q tests/w/test_prep_worker_mutants.py              # fast subset
    INFRX_MUTANTS=all uv run --frozen pytest -q tests/w/test_prep_worker_mutants.py
    (+ the lane's harness settings for the PostgreSQL list: see prep_worker_mutants.py)
"""
