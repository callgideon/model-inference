...
PY_MUTANTS: tuple[Mutant, ...] = (
    ...
    # --- round 3 (W4-confirm-b0a44a0): each reconciliation clause, the bench retry count,
    # a baseline short of levels, the printed setting
    _m("reconcile_ignores_failed", "the raw failed rows are the bench row's failed count",
       D, '                           and counted["failed"] == row.get("failed"))',
       "                           )", PASSING),
    _m("reconcile_ignores_accepted", "the raw accepted rows are the bench row's accepted count",
       D, '                           and counted["accepted"] == row.get("accepted")\n', "",
       PASSING),
    _m("reconcile_ignores_requests", "accepted + rejected + failed + cancelled = requests",
       D, 'and tally == row.get("requests")', "and True", PASSING),
    _m("bench_retries_ignored", "the bench row's retried_requests counts as retries",
       D, '        self.retried = sum(r.get("retries") or 0 for r in rows) + \\\n'
          '            ((row.get("denominators") or {}).get("retried_requests") or 0)\n',
       '        self.retried = sum(r.get("retries") or 0 for r in rows)\n', PASSING),
    _m("bench_retries_absent_read_as_zero", "a bench row without retried_requests is unknown",
       D, '        self.retries_known = all("retries" in r for r in rows) and \\\n'
          '            (row.get("denominators") or {}).get("retried_requests") is not None\n',
       '        self.retries_known = all("retries" in r for r in rows)\n', PASSING),
    _m("baseline_levels_unchecked", "a baseline without all six levels is not compared",
       D, "    elif missing or set(base.levels) != set(LEVELS):", "    elif False:", PASSING),
    _m("paired_levels_candidate_unchecked", "a candidate without all six levels is not compared",
       D, "    elif missing or set(base.levels) != set(LEVELS):",
       "    elif set(base.levels) != set(LEVELS):", PASSING),
    _m("setting_always_printed_not_taken", "a setting from a rule that passed is not flagged",
       D, 'taken = "" if rep["criteria"]["w3_rule"]["state"] == PASS else " (not taken)"',
       'taken = " (not taken)"', PASSING),
    _m("setting_printed_as_taken", "a setting from a rule not taken is printed as such",
       D, 'taken = "" if rep["criteria"]["w3_rule"]["state"] == PASS else " (not taken)"',
       'taken = ""', SET_ASIDE),
    # --- D10: the boundaries ---------------------------------------------------------------
...
SCRIPT_MUTANTS: tuple[Mutant, ...] = (
    ...
    _m("lock_by_file_existence", "the lock is released at exit, not left behind",
       C, 'exec 9>"$NVME/w4-candidate.lock" || refuse "cannot open $NVME/w4-candidate.lock"\n'
          'flock -n 9 || refuse "another candidate.sh holds $NVME/w4-candidate.lock"\n',
       '[ ! -e "$NVME/w4-candidate.lock" ] || refuse "another candidate.sh holds '
       '$NVME/w4-candidate.lock"\n: > "$NVME/w4-candidate.lock"\n', RESTORES),
    _m("ready_s_zero_accepted", "READY_S of 0 is refused",
       C, "[[ $READY_S =~ ^[1-9][0-9]*$ ]]", "[[ $READY_S =~ ^[0-9]+$ ]]", REFUSES),
    _m("lock_fd_inherited", "the candidate engine never holds the lock",
       C, '> "$OUT/engine-$1.log" 2>&1 9>&- &', '> "$OUT/engine-$1.log" 2>&1 &', RESTORES),
    _m("sampler_holds_the_lock", "the host sampler never holds the lock",
       C, 'host_sampler > "$OUT/host-mem-c$c.tsv" 9>&- &', 'host_sampler > "$OUT/host-mem-c$c.tsv" &',
       RESTORES),
    _m("health_on_another_port", "readiness is the engine's own /health on 127.0.0.1:8000",
       C, 'until curl -sf -m 5 "$ENGINE/health"',
       'until curl -sf -m 5 "http://127.0.0.1:8001/health"', RESTORES),
    _m("seqs_from_the_environment", "the sweep's engine runs 32 sequences, whatever the env says",
       C, "    ENGINE_MAX_NUM_SEQS=32 nohup bash", "    ENGINE_MAX_NUM_SEQS=${ENGINE_MAX_NUM_SEQS:-32} "
       "nohup bash", RESTORES),
