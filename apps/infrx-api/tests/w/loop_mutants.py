...
MUTANTS: tuple[Mutant, ...] = (
    ...
    _m("media_sent_as_a_bare_key", "S2M D3: the engine is handed a local file, not a key",
       E, '            return ({"type": VIDEO_PART,\n'
          '                     "video_url": {"url": local_media_url(ref, '
    ...
    _m("media_root_is_not_the_engines", "the root is the one the engine was started with",
       E, "    path = posixpath.normpath(posixpath.join(root, ref.storage_ref))",
       "    path = posixpath.normpath(posixpath.join(LOCAL_MEDIA_ROOT, ref.storage_ref))",
       MEDIA_FILE,
       # a path built under the wrong root fails the containment check, which is the kill
       allowed_errors=("NotFound",)),
    _m("media_path_unchecked", "the path must be inside the configured root",
       E, "    if not _inside_tenant_root(path, root, ref.org_id):", "    if False:",
       MEDIA_FILE),
    _m("media_path_any_tenant", "the path must be under this ref's own organization",
       E, "    prefix = posixpath.join(posixpath.normpath(root), STORAGE_REF_PREFIX, "
          "org_id) + \"/\"",
       '    prefix = posixpath.normpath(root) + "/"', MEDIA_FILE),
    _m("media_path_not_normalized", "a path is normalized before it is compared",
       E, "    return posixpath.normpath(path).startswith(prefix)",
       "    return path.startswith(prefix)", MEDIA_FILE),
    _m("media_relative_root_accepted", "a relative root cannot be compared, so it is refused",
       E, '    if not root.startswith("/") or not path.startswith("/"):', "    if False:",
       MEDIA_FILE),
