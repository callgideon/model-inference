...
SELF_TESTS = (
    ("a_syntax_error_is_not_a_kill", mutation_list.Outcome.broken_runner,
     mutation_list.Mutant(name="self_syntax", invariant="a broken copy is not a kill",
                          ...
                          cases=("test_api_stream__junk_and_stray_payloads_are_survived_not_relayed",))),
    ...
    # `health` is outside `generate`'s wrapper, so a NameError there really does escape
    # untyped - which is exactly the failure mode the rule must refuse to call evidence.
