...
def test_the_cutover_gate_needs_paused_admission_and_no_unmarked_preparing_job() -> None:
    ...
    def body():
        ...
        # 0023 (DOOR-REVOKE) took the doors without a marker from the runtime login ...
        assert gate()["runtime_unmarked_doors"] == [], gate()
        # ... and the gate still names them if a rollback of 0023 gives them back
        conn.execute("grant execute on function infrx.admit(jsonb), "
                     "infrx.claim_preparation(jsonb) to infrx_runtime")
