...
MUTANTS: tuple[Mutant, ...] = (
    ...
    _m("the_unjournalable_payload_is_sent", "a NUL or a non-finite number is refused typed",
       ...
       "        if False:", JSONB),
    _m("only_the_last_event_is_checked", "a bad event anywhere in the batch is refused (A1)",
       "        if not all(_journalable(event.payload) for event in events):",
       "        if not all(_journalable(event.payload) for event in events[-1:]):", JSONB),
    _m("a_lone_surrogate_is_sent", "a lone UTF-16 surrogate is refused (A2)",
       '        return "\\x00" not in value and not any("\\ud800" <= char <= "\\udfff" for char in value)',
       '        return "\\x00" not in value', JSONB),
    _m("a_json_scalar_is_refused", "ints, booleans and null are journalable (A4)",
