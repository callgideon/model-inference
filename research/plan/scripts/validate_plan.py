...
def consumer_closure_errors(manifest, tasks):
    """Check repair coverage and dispatch boundaries, not implementation success."""
    errors = []
    spec = manifest.get('consumer_v1_closure', {})
    mapping = spec.get('finding_tasks', {})
    expected = {f'RV-{n:02d}' for n in range(1, 13)}
    if set(mapping) != expected:
        errors.append('Consumer closure must map exactly RV-01 through RV-12')
    for finding, owners in mapping.items():
        if not owners or any(i not in tasks or tasks[i]['status'].startswith('superseded') for i in owners):
            errors.append(f'Invalid corrective owners: {finding}')
    for field in ('program', 'handoff'):
        if not spec.get(field) or not (PLAN / spec[field]).is_file():
            errors.append(f'Missing consumer closure {field}')
    roots = manifest['release_gates']['BACKEND-READY']['requires']
    if roots != [spec.get('backend_gate_task')] or roots != ['E4C']:
        errors.append('BACKEND-READY must use the current E4C corrective certificate')
    if manifest['release_gates']['BACKEND-LOCAL']['requires'] != ['E3C']:
        errors.append('BACKEND-LOCAL must use E3C corrective integration')
    if all(i in tasks for i in roots):
        backend = closure(tasks, roots)
        required = set('S3 F2C E2C D10 M5 M6 W5 G7 G8 E1C I8 E3C E4C'.split())
        if not required <= backend:
            errors.append('Backend gate omits corrective tasks: ' + ', '.join(sorted(required - backend)))
...
def ledger(manifest, tasks):
    app = closure(tasks, ['E4', 'I2A'])
    backend = closure(tasks, manifest['release_gates']['BACKEND-READY']['requires'])
...
def main():
    ...
    for t in rows:
        ...
        if t.get('disposition') in ('complete-plan-addition', 'backend-first-addition', 'consumer-v1-closure'):
    ...
    if not errors:
        ...
        frontend = set('A2 A3 C0 C3A U1 U1R U2 U3 U4 E3A I2A I3 E4 V1M'.split())
        ...
        errors.extend(consumer_closure_errors(manifest, tasks))
