...
## Backend-first gates and optimization oracles (2026-09-22)

The current release target is [the headless Marlin endpoint](18-marlin-backend-first.md). E3B is backend-local integration; E4B is the final real-GPU endpoint candidate. E3A/I2A/I3/E4 now reuse backend gates and add App-specific evidence later. Existing runtime/media/accounting oracles remain mandatory; these additions do not replace them.

| ID | Injection / action | Required passing observation | Owners |
|---|---|---|---|
| API-OPS | Provision/revoke keys, suspend, publish rates/deployment, repeat grant/adjustment and inspect/cancel jobs without a frontend | Operator-protected, audited and idempotent shared operations; exact per-user entitlement, no balance edits or privilege escalation | G6B/D/A1/E3B |
| BACKEND-JOURNEY | Two tenants make supported text/video URL/upload sync/SSE and explicit async calls; resume a bounded dataset client with App/Lab stopped | Actual auth/DB/storage/queue integration; accepted identities, output replay, cancellation, rate pins and exact usage reconcile; foreign/invalid/overloaded calls leave no leaked resources | E3B/G/M/Q/W/D |
| BACKEND-DEPLOY | Fresh target install/preflight, partial config/secret/migration failure, private engine probe | Pinned backend deploys independently; fail-closed startup, private engine, sanitized health and scoped external calls | I2B/I0/G/E4B |
| BACKEND-OBSERVE | Saturation, stalled preparation/generation/settlement, lease loss, full disk and service failure | Accurate phase/resource/loss/rejection/reconciliation metrics and actionable alerts without payload/secret disclosure; exercised recovery runbook | I3B/E4B |
| PERF-ENVELOPE | Diverse clip/frame/output profiles, cold/warm paths, open-loop sustained/burst/soak and paired direct-engine/gateway measurements | Raw denominators include errors/rejects; stable resource bounds; sample-supported percentiles; measured successful-work throughput and full cost at declared constraints | E1B/E4B |
| MEDIA-OPT | Before/after retrieval/decode/cache/preparation tuning with hostile/corrupt/expired media | Bounded resource use, tenant/security/retention intact, frame/time/processor semantics and model quality preserved or separately versioned; evidence supports chosen settings | M4/E4B |
