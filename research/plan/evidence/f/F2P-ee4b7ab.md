...
| `uv run --frozen pytest -q tests/contracts/v2/` | 0 | `268 passed` at `c42b213` (the v2 suites alone). **Corrected:** the first version of this report said 272, which was the console count transcribed into the wrong row. At the review-fix head the same command gives `269 passed` (one new case) |
...
### Three guards deleted because no single-edit mutant could kill them — RETRACTED

**This section's reasoning was wrong and the guards are restored in `8a513a8`; see “Review fixes” below.** It is kept as written so the record shows what was claimed.
...
| `research/plan/01a-contracts-v2-map.md` | prose map, §7 the wire-in file list, §8 the 15 proposed rulings, §9 the pending inputs |
...
5. `research/plan/08-contracts-v1-encoding.md` §10 — fold in the 15 proposed rulings (`01a` §8) with coordinator-assigned numbers; R60 was the last one this task read, and R61/R62 arrived mid-task.
...
**Unresolved findings for the coordinator:** (the three deleted guards were
reversed after review — see Review fixes); and `make check`
...
## Review fixes (independent review at `c42b213`: fix_required)

| Commit | Item |
|---|---|
| `8a513a8` | **B1** — restore the three guards. The retracted section above claimed the forged shapes were unconstructible; that is false, because pydantic's `model_copy(update=)` and `model_construct` skip mode="after" validators and `model_copy(update=)` is this tree's own idiom. The reviewer forged all three on unmodified `c42b213`: G1 a consumer `AuthContextV2` copied with the dev endpoint's `endpoint_id`/`provider_org_id` got pins for the private deployment; G2 a provider_dev `WalletRef` copied with the consumer's `owner_user_id`/`personal_org_id` was returned to the consumer credential; G3 a consumer `WalletRef` copied with the provider's `owner_provider_org_id` was returned to the provider_dev credential. Restored: `wallet.kind is not WalletKind.consumer` and `… provider_dev` (Forbidden, before the owner equalities) and `auth.audience is not CredentialAudience.provider_dev` inside the private branch (NotFound). The three named cases now carry exactly those `model_copy(update=)` shapes; mutants `pin_lets_a_copied_consumer_context_into_a_private_endpoint`, `a_consumer_credential_spends_a_copied_provider_wallet`, `a_provider_credential_spends_a_copied_consumer_wallet`: 3/3 killed. |
| `d3754b4` | **B2** — the three surviving reviewer mutants. (a) `digest_source` must be required (asserted in the r62 case; mutant `digest_source_defaults_to_confirmed`); (b) frozen records — new case `credit_rate__admitted_pins_are_immutable_and_every_pin_is_required` assigns in place and requires ValidationError (mutant `records_are_mutable`); (c) the same case pins the exact required-field sets of `AdmissionPins` and `AdmissionV2`, with only `schema_version`, `accounting_regime` (single-valued constants) and `replayed` allowed to default (mutants `a_pin_is_optional`, `admission_pins_are_optional`). 4/4 killed. |
| this commit | V3 amended (wallet kind of the credential's audience, checked explicitly), V15 added (pins immutable after acceptance; `model_copy(update=)`/`model_construct` not permitted on admitted pins — wire-in and D2–D5 enforce), wire-in notes (`NormalizedRequestV2.wallet_id` only from `resolve_wallet`; TS string comparison of timestamps relies on canonical `Z`), and the 272→268 count correction |

Re-run at `d3754b4` (UTC 2026-09-22), quoted tails:

| Command | Exit | Tail |
|---|---|---|
| `uv run --frozen pytest -q tests/contracts/v2/` | 0 | `269 passed in 12.14s` |
| `INFRX_MUTANTS=all uv run --frozen pytest -q tests/contracts/v2/test_mutants_v2.py` | 0 | `71 passed in 47.48s` (61 mutants + 4 list checks + 6 runner self-tests) |
| `uv run --frozen pytest -q --ignore=tests/d` | 0 | `1787 passed, 2 warnings in 169.95s (0:02:49)` |
| `node tests/contracts/v2/run-mutants-v2.mjs --self-test` / bare | 0 / 0 | `5/5 self-tests as expected` / `23/23 mutants killed` |
| `node --test tests/contracts/v2/*.test.ts` | 0 | `# tests 17 / # pass 17 / # fail 0` |

The v2 conformance suite is now 32 cases; the Python v2 list 61 mutants. The
console half was not affected by B1/B2 (the TS derivations take no model-copied
records), so its counts are unchanged. tests/d and `make integration` remain not
run for the reasons above.
...
- 2026-09-22: Independent review at `c42b213` returned fix_required. The v2 test
  count was corrected (272 → 268; the console count had been transcribed), the
  deleted-guards section was retracted in place rather than removed, and the
  "Review fixes" section records B1/B2 and the re-run tails.
