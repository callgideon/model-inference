...
2. **Coordinator / F (contracts): apply WITH the D5 merge. Otherwise `make api-test` and every contracts mutant are red.**
   ...
   - Fix round: the diff below is unchanged (sha256 `a8e75c3f…`). `git apply --check` passes against this branch's merged head, where the `mutants.py` hunk now sits 78 lines lower (l.2159). On the trial merge with the phase-3 lane it reverse-applies: it is already applied there, as `8c00bb4`. Apply it once, through that lane. The re-verification on a patched copy is under "Fix round runs".
3. **G2.**
...
4. **Coordinator / G6B.**
   ...
   - **Fix round: superseded by the cutover lane. Do NOT apply this diff.** Cutover item 8 (`f7d9b03`, "the operator tool builds D5's PostgreSQL adapters from the environment") implements `build_operations(settings=None)` with the same composition, reading `settings.pilot.database_url`. It reaches the phase-3 branch through its cutover merge. On the trial merge (this branch + phase-3 `1fa825b`), the diff applies neither forward nor in reverse (`git apply --check`: `patch failed: apps/infrx-api/infrx/operations/cli.py:10`). So the merge plan's step 2(a) (`2-e3b-phase3-bodies.sh`, `git apply --index d5-cli-build-operations.diff`) now fails and must be dropped. Against this branch alone, the diff still applies.
...
14. **A CONCURRENT reuse of one `reconcile` operation id dies untyped** (verifier V-N1).
    - Two requests racing with one operation id: the loser fails on `audit_entries_idempotency_key_idx` (0009) as an untyped 23505 (a 500).
    - This fails safe: the loser's hold stays `unknown`, and nothing moves.
    - The sequential reuse is typed `idempotency_conflict` and pinned (`d5_reconcile_replay_any_request`), so 0018's header sentence holds sequentially only.
    - The fix is `grant_credit`'s N5 pattern: catch `unique_violation` on reconcile's audit insert, plus a lock-step case and a mutant. It is a post-checkpoint follow-up by the coordinator's ruling; 0018 is not changed now.
...
- **Unresolved findings:** Limits 1–4, 6, 7, 12, 13, 14. Limit 1 is gone once the phase-3 lane's `8c00bb4` is in the merged tree.

## Fix round (review `D5-review-c67e4f5.json`, fix_required)

The review of `c67e4f5` (implementation `4bcac3b`) returned 8 confirmed blocking findings, 2 downgraded and 17 nonblocking. Its refuters confirmed every money path correct. The blocking items were invariants that no test could catch, plus merge readiness. This was the ONE fix round. Every finding has a commit, or an evidence change that says why not.

- **Merges.**
  - `c807bb5` merged `origin/claude/backend-impl` `d926cc9`. Its one conflict was the partition's `RAISES` (review CF-6). It was resolved by taking D5's lifted partition whole: `RAISES` names the F2 case only, and the integration head's `75cd7bb`/`f52308a` entries are dropped with their PENDING lines. `tests/d/test_journal_units.py` auto-merged.
  - `493c4ac` merged `0bdb61a`. That merge touched plan, evidence and results only, and had no conflict.
- **Code SHA of the round:** `8554b47`. Runs F1–F14 ran on heads `493c4ac` … `4bfdbf0`, whose `apps/` trees equal `8554b47`'s.
- **The verifier's pins** (`D5-verify-4bfdbf0.json`, PASS with nonblocking V-N1…V-N6) are the only later change: test files only, no SQL or adapter change. They are in the same commit as this section, and runs F15–F23 are at that tree.
- Status stays **implemented**.
- **Session note.** The first fix-round implementer's session ended (the Opus session-limit outage) after `8554b47`, while its verification battery ran. A second implementer resumed from the committed state and the scratchpad drafts. It re-ran the whole battery on heads `493c4ac` … `4bfdbf0` (runs F1–F14; those heads differ only in this evidence file, so their `apps/` trees equal `8554b47`'s). The earlier batteries at `8554b47` are superseded and not quoted: one of them read a stale `.pyc` of a hand-typo proof, and one hit a port collision.

### Per finding: commit → case → mutant kill text

Migration mutants are single edits of 0018, killed by an `AssertionError` in the named check; the kill texts are quoted from `-s` runs F9/F10 on **both images**. Code mutants go through the shared runner (R83): a kill is the named declared case failing (F11). A "hand-applied" line is the same edit applied to the worktree and run against the named PostgreSQL case, then reverted. Those texts were quoted by the first fix-round session in the commit message named, and were not re-run.

| Finding | Commit | Change | Killing check / case | Mutant(s) → kill text |
|---|---|---|---|---|
| **B1**: `grant_credit`, only the AMOUNT part of "another movement" was tested | `369137d` | `check_adjust` reuses one operation id for another amount, ANOTHER WALLET (CONSUMER_2's) and ANOTHER KIND (`operator_allocation`). Each is `idempotency_conflict`; the ledger and both wallets are unchanged | `adjust` (`test_operations_pg.py::test_credit_spend__adjust_is_audited_idempotent_and_never_below_reserved`) | `d5_grant_replay_ignores_wallet` (the reviewer's `rv_replay_ignores_wallet`) → `AssertionError: the operation id reused for another wallet answered (None, {'entry': {'kind': 'operator_adjustment', 'unit': 'CREDIT', 'actor': 'ops@t…` (both images); `d5_grant_replay_ignores_kind` (`rv_replay_ignores_kind`) → `AssertionError: the operation id reused for another kind answered (None, {'entry': {'kind': 'operator_adjustment', 'unit': 'CREDIT', 'actor': 'ops@tes…` (both images) |
| **B2**: the wallet `FOR UPDATE` had no race test | `df328c8` | `check_settle_races`: one operation id twice on ONE wallet, lock-step. The second call waits on the wallet row, then answers `replayed: true` with the first entry. Exactly one ledger row | `settle_races` (`test_settle_races.py::test_races__the_mutants_concurrency_check`) | `d5_grant_without_wallet_lock` (`rv_grant_without_wallet_lock`) → `AssertionError: a racing replay was refused: idempotency_conflict` (both images) |
| **N5**: a concurrent reuse on another wallet died of an untyped 23505 | `b968cc8` | 0018 `grant_credit`: the ledger insert catches `unique_violation` and refuses the typed `idempotency_conflict`, because the wallet lock serializes one wallet only. Pinned by lock-step in `check_settle_races` and by the barrier race `test_race__one_operation_id_is_one_movement_under_a_barrier`: one row; the winner's wallet replays; the other conflicts | `settle_races` | `d5_grant_concurrent_reuse_untyped` → `AssertionError: one operation id on two wallets at once: None, untyped 23505` (both images) |
| **B3 / H-B2 / CF-8** (downgraded): `reconcile`'s replay was keyed on the operation id alone | `833e9a1` | 0018 `reconcile` reads the audit row. If the row names another request, the call is refused `idempotency_conflict`, as in `grant_credit`. `check_reconcile_clock`: the id reused for another `held_unknown` request past its window is a conflict; that hold stays `unknown` and nothing is audited | `reconcile_clock` (`test_operations_pg.py::test_dur_settle__reconcile_waits_for_the_db_clock_and_never_debits`) | `d5_reconcile_replay_any_request` → `AssertionError: an operation id reused for another request answered: None` (both images); and `d5_reconcile_replay_audits_again`, re-anchored on the new read → `AssertionError: a replayed reconcile was refused: untyped 23505` (both images) |
| **CF-1**: `move_alias`'s card choice was never exercised | `6047d3d` | The alias-move case puts two cards on the moved deployment (one an hour older) and asserts that `infrx.resolve_admission_pins` answers the NEWEST, and that the next admission pins it. The case calls `ensure()`, so it runs alone. A unit reads the card subquery `move_alias` sends | `test_catalog_pg.py::test_credit_rate__alias_move_changes_resolve_not_an_admitted_job`; unit `test_operations_units.py::test_registry__the_alias_moves_at_the_deployments_newest_effective_card` | code `alias_moves_at_the_oldest_card` (the reviewer's R04) → `[killed] alias_moves_at_the_oldest_card: 1 failed, 14 deselected in 0.75s`. Hand-applied on PostgreSQL (`6047d3d`): `AssertionError: the alias moved at an old card` |
| **CF-2**: nothing read the actor the adapters send | `6e36f21` | The ledger-port case asserts `entry.actor == "ops@test"` and that the `grant_credit:<op>` audit row's `actor_principal` is the caller. The service case asserts that the `admin_adjust`/`admin_reconcile` rows carry the principal the service audits under. A unit reads the actor both adapters send | `test_operations_pg.py::test_api_ops__the_ledger_port_adjusts_reconciles_and_grants_through_d5_and_a1`, `…__the_operations_service_runs_on_the_postgres_adapters`; unit `test_ledger__asks_for_an_operator_adjustment_and_answers_the_entry` | code `adjust_sends_another_actor` (R03) → `[killed] adjust_sends_another_actor: 1 failed, 14 deselected in 0.70s`; `reconcile_sends_another_actor` (R02) → `[killed] reconcile_sends_another_actor: 1 failed, 14 deselected in 0.71s`. Hand-applied on PostgreSQL (`6e36f21`): both fail the named cases |
| **CF-3**: `resolve`'s state predicates were untested | `ef01549` | New PostgreSQL case: a public deployment set `draining` and a private one set `retired` each resolve None. A unit reads both predicates in the statements sent | `test_catalog_pg.py::test_credit_rate__a_draining_or_retired_deployment_does_not_resolve`; unit `test_catalog__a_private_deployment_only_for_its_provider_and_errors_raised` | code `public_resolves_an_inactive_deployment` (R05) → `[killed] public_resolves_an_inactive_deployment: 1 failed, 14 deselected in 0.80s`; `private_resolves_a_retired_deployment` (R06) → `[killed] private_resolves_a_retired_deployment: 1 failed, 14 deselected in 0.69s`. Hand-applied on PostgreSQL (`ef01549`): `a draining deployment still resolves its public alias` / `a retired private deployment still resolves` |
| **CF-4**: the fresh-thread case could not see a kept connection | `1072658` | The case answers on the main loop first, then on two threads at once, each with its own loop. A unit spies on the `Connect`: one connection is opened and one closed per statement, and none is reused | `test_catalog_pg.py::test_credit_rate__a_lookup_from_a_fresh_thread_and_loop_is_answered`; unit `test_catalog__every_lookup_opens_and_closes_its_own_connection` | code `a_connection_cached_across_calls` → `[killed] a_connection_cached_across_calls: 1 failed, 14 deselected in 0.78s`; `a_connection_never_closed` → `[killed] a_connection_never_closed: 1 failed, 14 deselected in 0.75s`. Hand-applied R09 (one kept connection) on PostgreSQL (`1072658`): `AssertionError: {0: RateCardSnapshot(…)}`, meaning one thread never answered |
| **CF-5 / H-B1**: merge readiness, the integration head's E3B/I3B tripwires on D5 | evidence `cf1c315` | D5 wrote nothing here, by instruction. The E3B phase-3 lane (`codex/e3b-phase3-bodies`, stacked on `c67e4f5`) wrote dr07c and rc04b, retired the D5 pendings, and applied requests 1, 2 and 8. Integration request 1 lists every item with its phase-3 commit. D5 merges TOGETHER with that lane. The round-1 line "make integration not needed" is corrected in place. The pgstate rows for 0018 are unchanged by this round (no new object) | - | - |
| **N1**: the grids could not tell exact numeric from float | `524f509` | Both grids gain a high-rate point: 71665.96953467 / 27893.11916121 per M at (17318, 1176) costs exactly 1273.91356853. Binary floating point inside PostgreSQL (float8, cast back to numeric at 15 significant digits) gives …54; Python's float would round to …53 (the verifier's note). The CREDIT card `rc_d5_float` goes through `debit_credit`, and the legacy snapshot goes through `debit_legacy_usd` | `credit_grid`, `settle_exact` | `d5_credit_debit_in_float` (`rv_credit_debit_in_float`) → `AssertionError: ('rc_d5_float', 17318, 1176, Decimal('1273.91356854'))` (both images); `d5_legacy_debit_in_float` → `AssertionError: (('71665.96953467', '27893.11916121'), 17318, 1176, Decimal('1273.91356854'), Decimal('1273.91356853'))` (both images) |
| **N2**: the lookup's OPERATION part was untested | `bbc2be3` | `check_lookup`: the same key under another operation answers `(None, None)` | `lookup` (`test_settle.py::test_dur_admit__lookup_reads_the_mapped_job_and_writes_nothing`) | `d5_lookup_ignores_operation` (`rv_lookup_ignores_operation`) → `AssertionError: the same key under another operation answered this operation's job` (both images) |
| **N3**: the STABLE marker was asserted nowhere | `337f572` | `check_d5_privileges` asserts `pg_proc.provolatile = 's'` for `infrx.idempotency_lookup(jsonb)` | `d5_privileges` | `d5_lookup_not_stable` → `AssertionError: infrx.idempotency_lookup is not STABLE: nothing stops a write inside it` (both images) |
| **N4**: only low-precision amounts were round-tripped | `1ec4972` | `check_allocation` allocates `123456789012.12345678` (12 integer digits, 8 places). The entry's amount text and the wallet total are both exact | `allocation` | `d5_amount_through_float` → `AssertionError: (None, {'entry': {'kind': 'operator_allocation', 'unit': 'CREDIT', 'actor': 'ops@test', 'amount': '123456789012.12300000', 'reason': '…` (both images) |
| **N6**: the units covered only settled and released_free, and not the org lookup sends | `1ad4e3c` | The SETTLEMENT unit adds `held_unknown` and `released_platform_absorbed` CREDIT outcomes (no `SettlementV2`). The lookup unit checks that the CALLER's org is sent even when the scope names another | units `test_complete_credit__a_settlement_exactly_when_settled_at_the_recorded_charge`, `test_lookup__answers_the_jobs_own_regime_and_sends_the_stores_ttl` | code `a_settlement_for_an_unsettled_outcome` (`rv_code_settlement_on_any_state`) → `[killed] a_settlement_for_an_unsettled_outcome: 1 failed, 14 deselected in 0.70s`; `lookup_sends_the_scopes_org` (`rv_code_lookup_sends_the_scopes_org`) → `[killed] lookup_sends_the_scopes_org: 1 failed, 14 deselected in 0.72s` |
| **N7 / CF-7 / H-N3**: `result_expires_at` on the owned read | evidence `4b7e321` | **Stated, not implemented.** It needs a field on `TerminalOutcome`, which is F's frozen contract, so it is outside D5's paths and not small. "G3 can drop its route-side TTL" is corrected in place. The three-part proposal is under "Further deltas". G3 request (c) stays open (Limits 12) | - | - |
| **CF-6**: the partition conflict with `f52308a` | `c807bb5` | Resolved as above, and restated in integration request 1 | `test_the_pending_list_names_only_real_cases` (`set(RAISES) == set(PENDING)` refuses a naive union) | - |
| **H-N1**: nothing proved `assert_no_drift` can fail | `cf2ab2c` | The positive control: as the owner, with the wallets' guards off and inside a rolled-back transaction, a USD and a CREDIT wallet total are written past their ledgers. `assert_no_drift` must then fail, naming both detectors | `test_settle.py::test_dur_settle__assert_no_drift_fails_on_an_induced_drift` | passes on both images (F7, F8): the control's own assertion requires the failure (`both detectors report an induced drift`) |
| **H-N2**: five `why` texts described deaths their runs did not show | `5c3802a` | Each `why` now states the observed death. `check_settle_late_data` asserts the foreign/stale lease on a LIVE job first | `adjust`, `settle_causes`, `settle_envelope`, `settle_terminal_event`, `settle_late_data` | `d5_settle_before_the_fence` → `AssertionError: a foreign worker settled: None` (both images) (for the fence now); `d5_adjust_replay_appends` → `AssertionError: a replayed adjustment was refused: idempotency_conflict` (both images); `d5_sync_deadline_billed` → `AssertionError: ('sync_deadline', 'untyped 23514')` (both images); `d5_debit_above_hold` → `AssertionError: a debit past the hold failed instead of being absorbed: untyped 23514` (both images); `d5_second_terminal_event` → `AssertionError: untyped 23505` (both images) |
| **H-N4**: the relay's lookup-refusal fallback is dead too | evidence `4c6f7bb` | Named in integration request 3: `relay.py:204-211`, its two `test_relay_recovery` cases, and the g mutant `lookup_refusal_escapes`. G3/E4B removes them | - | - |
| **H-N5**: a typo in ALWAYS would drop a mutant silently | `8554b47` | `test_the_mutant_list_is_well_formed` refuses an ALWAYS name the list does not declare. `ALWAYS` += the round's six money-path mutants | `test_the_mutant_list_is_well_formed` | a hand typo on a scratch copy (F13): `AssertionError: ALWAYS names mutants the list does not declare: ['d5_legacy_debit_in_flaot']` |
| **H-N6**: the evidence was WIP, and a mutant was mislabelled | `28237a3` (rows 20–25 quoted), `9988176` | `errors_become_none` edits `operations._Db.rows`, which the catalog uses; the item-8 row now says so | - | - |
| **H-N7**: request 1 was still unapplied on the head | evidence `cf1c315` | Restated in integration request 1: applied by the phase-3 lane (`54d3955`, `eb08901`) | - | - |
| **V-N2** (verifier): the actor/reason half of the replay rule was unpinned | this commit | `check_adjust` replays the op with `actor='other@test'` and another reason. The answer is `replayed: true` with the FIRST entry, the actor stays `ops@test`, and nothing is appended | `adjust` | `d5_grant_actor_is_part_of_the_movement` (the verifier's om1) → `AssertionError: a replay by another actor was refused: idempotency_conflict` (both images) |
| **V-N3** (verifier): the connection spy covered only successful statements | this commit | The spy case adds a statement that raises (`_db_error('55000')`). `DependencyUnavailable` is raised typed, and `closed == opened` (5 of 5) | unit `test_catalog__every_lookup_opens_and_closes_its_own_connection` | code `a_connection_leaks_on_error` (om6) → `[killed] a_connection_leaks_on_error: 1 failed, 14 deselected in 0.73s`. Hand-applied on a scratch copy (F21): `AssertionError: a failed statement leaked its connection` |
| **V-N4** (verifier): the positive control induced only ledger drift | this commit | `check_drift_detected` restores the ledger, then writes `reserved_total + 1` on the USD and the CREDIT wallet (guards off, rolled back). `assert_no_drift` must fail naming both detectors. New check key `drift_detected` | `test_settle.py::test_dur_settle__assert_no_drift_fails_on_an_induced_drift` | `d5_credit_reserved_drift_blind` (om8: 0006's `credit_wallet_reconciliation.reserved_drift` → 0) → `AssertionError: a reserved total written past its active holds: the wallet summary drifted from its ledger and holds: USD [(UUID(…), Decima` (both images) |
| **V-N1** (verifier): a CONCURRENT reuse of one reconcile operation id dies untyped 23505 on the audit key | - | **Not changed, by the coordinator's ruling** (a post-checkpoint follow-up). It fails safe: the loser's hold stays `unknown` and nothing moves. The sequential reuse is typed and pinned (`d5_reconcile_replay_any_request`). 0018's header sentence holds sequentially only. See Limits 14 | - | - |
| **V-N5** (verifier): no Fix round section or run quotes | this commit | This section | - | - |
| **V-N6** (verifier): merge-plan step 2(a) still applies the cli diff | coordinator | Drop step 2(a) (request 4 is superseded by `f7d9b03`; F14) | - | - |

Also changed in the fix round:

- **The three diffs, re-checked (`3ae9834`).** Request 2 (contracts retirement) applies at the merged head with a 78-line offset, and is already applied on the phase-3 lane (`8c00bb4`). Request 4 (cli) is **superseded** by cutover `f7d9b03`: on the phase-3 head it applies neither forward nor in reverse, so merge-plan step 2(a) must be dropped. `q3rig.diff` applies on both. All three are embedded below unchanged: sha256 `a8e75c3f…`, `d2a26348…`, `4b69278e…`, the last two as the merge plan checks them.
- **Totals.** **72** D5 migration mutants (60 + 10 in the review round + 2 verifier pins, `d5_grant_actor_is_part_of_the_movement` and `d5_credit_reserved_drift_blind`; the ten: `d5_grant_replay_ignores_wallet`, `d5_grant_replay_ignores_kind`, `d5_grant_without_wallet_lock`, `d5_grant_concurrent_reuse_untyped`, `d5_reconcile_replay_any_request`, `d5_lookup_ignores_operation`, `d5_lookup_not_stable`, `d5_amount_through_float`, `d5_credit_debit_in_float`, `d5_legacy_debit_in_float`) and **38** D5 code mutants (28 + 9 + `a_connection_leaks_on_error`). The whole migration list declares **549**, and the default subset takes **168**: both pins joined `ALWAYS`. F1/F2 counted 547/166 before the pins; F20 counts after them.

### Fix round runs (from `apps/infrx-api`; the same task-local environment; driver `scratchpad/d5/driver4.sh` + `driver5.sh`; logs `scratchpad/logs/r4-*`)

The driver waits until no socket holds a task port before each step (see Limits 13). It retries once, after 60 s, on `address already in use` (P-21). The `dirty` count in each log's first line is this evidence file only: nothing under `apps/` differed from `493c4ac`.

| # | Command | Tree | UTC | Exit | Tail (quoted) | Log |
|---|---|---|---|---|---|---|
| F1 | `pytest -q -s -k well_formed tests/d/test_migration_mutants.py` (default subset) | `493c4ac` | 23:17:18Z → 23:17:18Z | 0 | `547 mutants over 120 checks; 166 selected (subset)` | `r4-count-subset.log` |
| F2 | the same with `INFRX_MUTANTS=all` | `493c4ac` | 23:17:19Z → 23:17:19Z | 0 | `547 mutants over 120 checks; 547 selected (all)` | `r4-count-all.log` |
| F3 | `pytest -q tests/d/test_code_mutants_d5.py tests/d/test_settle_units.py tests/d/test_operations_units.py` | `493c4ac` | 23:17:19Z → 23:18:53Z | 0 | `53 passed in 93.28s (0:01:33)` | `r4-code-mutants.log` |
| F4 | `pytest -q -rf tests/contracts` | `493c4ac` | 23:18:53Z → 23:19:40Z | 1 | `23 failed, 1033 passed in 46.18s`. Red for Limits 1 only: exactly `test_cancel_cause.py::test_dur_settle__before_0018_the_pg_store_refuses_a_cause_it_cannot_record` (1) + `test_mutants.py` (22, `broken_runner` at the pristine baseline). F12 shows it green with request 2 applied | `r4-contracts.log` |
| F5 | `pytest -q tests/g` | `cf1c315` | 23:20:41Z → 23:23:13Z | 0 | `559 passed, 2 warnings in 149.84s (0:02:29)` | `r4-g.log` |
| F6 | `pytest -q -x tests/w` | `4b7e321` | 23:23:13Z → 23:34:00Z | 0 | `180 passed in 647.14s (0:10:47)` | `r4-w.log` |
| F7 | full D sweep, plain: `INFRX_MUTANTS=all pytest -q -rsx tests/d` | `4bfdbf0` | 23:34:00Z → 00:01:23Z | 0 | `1051 passed, 5 xfailed in 1640.45s (0:27:20)`. The 5 XFAIL are the partitions; every D1–D5 and A1 mutant killed | `r4-sweep-plain.log` |
| F8 | D sweep, Supabase (`INFRX_D1_IMAGE=supabase`), default subset: `pytest -q -rsx tests/d` | `4bfdbf0` | 00:02:24Z → 00:20:36Z | 0 | `658 passed, 5 xfailed in 1089.78s (0:18:09)`. The same 5 XFAIL; every subset mutant killed | `r4-sweep-supabase.log` |
| F9 | `INFRX_MUTANTS=all pytest -q -s tests/d/test_migration_mutants.py -k "d5_ or d3_cancel or d3_claim or d3_unknown_release"`, plain | `4bfdbf0` | 00:25:07Z → 00:27:32Z | 0 | `92 passed, 460 deselected in 143.33s (0:02:23)` | `r4-kills-plain.log` |
| F10 | the same, Supabase | `4bfdbf0` | 00:33:24Z → 00:35:45Z | 0 | `92 passed, 460 deselected in 140.15s (0:02:20)` (retried once: P-21) | `r4-kills-supabase.log` |
| F11 | `python -m tests.d.code_mutants_d5` (all 37, the shared runner) | `4bfdbf0` | 00:37:47Z → 00:39:14Z | 0 | `37/37 killed` | `r4-code-kills.log` |
| F12 | `pytest -q tests/contracts` on a scratch COPY of the head with `contracts-retire-0016-refusal.diff` applied (`patch -p1`; the worktree untouched) | `4bfdbf0` | 00:22:37Z → 00:24:07Z | 0 | `1055 passed in 88.29s (0:01:28)` | `r4-contracts-copy.log` |
| F13 | H-N5 proof: `pytest -q -k well_formed tests/d/test_migration_mutants.py` on a scratch copy whose ALWAYS says `d5_legacy_debit_in_flaot` | `493c4ac` | 23:19:24Z | 1 | `1 failed, 169 deselected in 0.46s`. The intended red: `AssertionError: ALWAYS names mutants the list does not declare: ['d5_legacy_debit_in_flaot']` | `r4-typo-proof.log` |
| F14 | the three diffs: `git apply --check` (forward and `-R`) against the head and against the scratch trial merge with `origin/codex/e3b-phase3-bodies` `1fa825b` | `4bfdbf0` | 23:27:24Z → 23:27:24Z | 0 | `Hunk #1 succeeded at 2159 (offset 78 lines).`. On the head all three apply forward (rc=0). On the trial merge: contracts forward rc=1 / reverse rc=0 (already applied, `8c00bb4`); cli forward rc=1 / reverse rc=1 (superseded by `f7d9b03`); q3rig forward rc=0 | `r4-diffs.log` |
| F15 | the coordinator's isolated rerun: `INFRX_MUTANTS=all pytest -q -s tests/d/test_migration_mutants.py -k d1r_grant_race_arbitrates_one_index`, Supabase | `4bfdbf0` + the pins (this commit) | 00:43:15Z → 00:43:22Z | 0 | `1 passed, 553 deselected in 5.35s`. Killed in isolation (below) | `r4-d1r-race-supabase.log` |
| F16 | the verifier pins' cases, pristine: `pytest -q -s tests/d/test_operations_pg.py tests/d/test_settle.py -k "adjust_is_audited or assert_no_drift_fails"`, plain | `4bfdbf0` + the pins (this commit) | 00:44:24Z → 00:44:30Z | 0 | `2 passed, 31 deselected in 4.27s` | `r4-vn-pins-plain.log` |
| F17 | the same, Supabase | `4bfdbf0` + the pins (this commit) | 00:45:33Z → 00:45:40Z | 0 | `2 passed, 31 deselected in 6.81s` | `r4-vn-pins-supabase.log` |
| F18 | `INFRX_MUTANTS=all pytest -q -s tests/d/test_migration_mutants.py -k "d5_grant_actor_is_part_of_the_movement or d5_credit_reserved_drift_blind or well_formed or superseded or supersession"`, plain | `4bfdbf0` + the pins (this commit) | 00:46:43Z → 00:46:51Z | 0 | `7 passed, 547 deselected in 7.12s` | `r4-vn-kills-plain.log` |
| F19 | the same, Supabase | `4bfdbf0` + the pins (this commit) | 00:47:52Z → 00:48:14Z | 0 | `7 passed, 547 deselected in 21.21s` | `r4-vn-kills-supabase.log` |
| F20 | `pytest -q -s -k well_formed tests/d/test_migration_mutants.py` (default subset) after the pins | `4bfdbf0` + the pins (this commit) | 00:49:14Z → 00:49:15Z | 0 | `549 mutants over 121 checks; 168 selected (subset)` | `r4-vn-count-subset.log` |
| F21 | `a_connection_leaks_on_error` hand-applied to a scratch copy: `pytest -q -k every_lookup_opens tests/d/test_operations_units.py` | `4bfdbf0` + the pins (this commit) | 00:43:00Z | 1 | `E       AssertionError: a failed statement leaked its connection`. The intended red: the mutant dies by the case's own assertion | `r4-vn3-hand.log` |
| F22 | `pytest -q tests/d/test_code_mutants_d5.py tests/d/test_settle_units.py tests/d/test_operations_units.py` after the pins | `4bfdbf0` + the pins (this commit) | 00:49:15Z → 00:50:32Z | 0 | `54 passed in 76.70s (0:01:16)` | `r4-vn-code.log` |
| F23 | `python -m tests.d.code_mutants_d5` after the pins (all 38) | `4bfdbf0` + the pins (this commit) | 00:50:32Z → 00:51:48Z | 0 | `38/38 killed` | `r4-vn-code-kills.log` |

Notes on the runs:

- **F10's first attempt was the P-21 collision.** It hit `could not start infrx-d5-postgres-supabase: … failed to bind host port 127.0.0.1:55436/tcp: address already in use` 52 times (`r4-kills-supabase.try1.log`: `52 failed, 40 passed`, every failure that same bind error). The driver waited 60 s and retried once: `92 passed`, every mutant killed.
- **`d1r_grant_race_arbitrates_one_index` (D1R's 0006 race mutant): load-sensitive, not a D5 finding.** It is not in `ALWAYS`, so F8's Supabase subset does not run it. The coordinator reported it SURVIVED on the Supabase image in a run under host load ~11. Rerun alone on the Supabase image (F15) it is killed by its named check: `killed by grant_race -> AssertionError: a personal-org-only collision answered '23505', not the typed rollout hold (55000): the insert does not absorb a conflict on the per-personal-org index`. F7 (plain, every mutant) also killed it. It is recorded like W3's load-sensitive sigint mutant: a race check whose interleaving is not forced under heavy load. The owner is D1R, and it is unchanged by D5.
...
- 2026-09-24 (D5, fix round, Opus implementer resumed after the session-limit outage): the review `D5-review-c67e4f5.json` (fix_required) was answered in one fix round, with code at `8554b47`. The verifier `D5-verify-4bfdbf0.json` gave PASS; its nonblocking pins V-N2/V-N3/V-N4 are test-only and in this commit, V-N1 is Limits 14, V-N5 is the "Fix round" section, and V-N6 is the coordinator's merge-plan step 2(a). The round-1 sections are kept as written, except three statements the review showed were wrong. They are corrected in place and say so: "make integration not needed", "G3 can drop its route-side TTL", and request 4's diff, now superseded. Every tail in the fix-round runs table (F1–F23) is quoted from the named `r4-*` log. Nothing was pushed by D5, and nothing ran outside the task-local containers.
