...
Nothing else was started, stopped or inspected. E2's `infrx-e2-*` stack was running on its
own ports while this task ran and was never touched by it (it is no longer listed now,
which is its own lane's doing, not this one's); the container created here was removed at
...
| 7 | `INFRX_MUTANTS=all uv run --frozen pytest -q -p no:cacheprovider tests/contracts/test_mutants.py tests/m/test_mutants.py tests/q/test_mutants.py tests/q/test_valkey_mutants.py tests/j/test_mutants.py tests/w/test_mutants.py tests/t/test_trace_mutants.py tests/g/test_mutants.py` | 0 | 17:12:09 → 17:36:20 |
| 8 | `INFRX_Q2_DIFFERENTIAL=all uv run --frozen pytest -q -p no:cacheprovider tests/q/test_valkey_scheduler.py` | 0 | 17:36:20 → 17:38:08 |
...
| 7 — every mutant list (minus D) | `1035 passed in 1450.86s (0:24:10)` | every declared mutant of every track's list killed, Q2's 76 among them |
| 8 — suite with the full differential (the 40 × 2,500 streams inside the suite) | `44 passed in 107.88s` | all pass |
...
Committed files (git blob hashes at `10571c7`, regenerated from `git ls-tree 10571c7` after review found two stale rows; the r2 table is in the revision section):
...
| `apps/infrx-api/tests/q/test_valkey_scheduler.py` | `cdb76c96677966f08f13a49e8410e1c67d9b984e` |
| `apps/infrx-api/tests/q/valkey_mutants.py` | `c86e8ef97834fff14d76bc0135bb16f404cd8375` |
...
ec6c548 10571c7 -- . ':!research'` = `7 files changed, 2658 insertions(+), 1
deletion(-)` (an earlier draft said 2639, hand-typed; corrected from command output).
...
2. `apps/infrx-api/infrx/contracts/tasklocal.py`: add `"q2": {"valkey": 55461}` to
   ...
   **r2:** the coordinator is adding this entry under R63 on `claude/backend-impl`; the
   harness now reads `local_services("q2")` and falls back to 55461 only while the key
   is absent.
...
* **Test namespaces are not garbage collected.** Every harness takes a fresh namespace
  and no test deletes it, because the container is disposable; measured during this
  session the server held `dbsize` 12,111 keys at `used_memory_human:24.48M`, and
  `dbsize` 16,009 immediately before removal, after roughly 300,000 indexed operations. A long-lived server driven by a test loop
...
## Revision r2 — commit-fence review (fix_required at `10cd1c1`)

The independent review found the code correct but the select-then-commit fence
untested: two single-edit mutants of the fence line survived all 44 cases. Fixed on
this branch, one commit per item; history not rewritten.

| Field | Value |
|---|---|
| Implementation SHA (r2) | `a42bd856c37346b22f3e40df05ad5cd5a4b3a3de` |
| Commits | `a1d80e2` concurrent-dispatcher case + two fence mutants · `0c9d85a` refuse a committed claim that is not the priced candidate · `a42bd85` harness reads the task-local table, readiness waits for PONG |

| Point | Test id | Exact invariant demonstrated |
|---|---|---|
| fence | `test_q2_race__two_dispatchers_never_receive_one_candidate` | two adapters (separate clients, one namespace, one clock), 8 concurrent claim loops (4 `kind=None`, 4 `kind=INFER`) over 60 candidates (3 orgs × 2 kinds): every handed-out id unique and all 60 handed out, `stats()["inflight"] == len(handed)`, and every state-2 reply's id equals the priced `expected` argument (checked by wrapping `_call`). `_CLAIM_ATTEMPTS` exhaustion is retried by the loop, as Q3 must |

New mutants (anchor: the fence line of `_CLAIM`, `if expected == '' or expected ~= pick.id then return {1, pick.id, payload} end`), both naming the case above:
`the_commit_skips_the_fence` (commit whatever the fair choice now is) and
`the_commit_trusts_expected_even_if_no_longer_pending` (commit the priced id even
after another dispatcher took it). Standalone, run five times in a row with the
container warm: `2/2 killed` each time. The case itself passed 10/10 consecutive runs
on the correct code.

Hardening (`0c9d85a`): a state-2 reply whose id differs from the priced one raises
`InternalError` in Python. It is defence in depth behind the script's fence and is
unreachable with a correct script, so no mutant claims it (a mutant deleting it would
survive by construction; the Lua fence is the claimed invariant).

Harness (`a42bd85`): container name and port come from
`tasklocal.local_services("q2")["valkey"]`, with the 55461 fallback while `"q2"` is
not in `TASK_PORTS`. Readiness now waits for a `+PONG`: the first cold-start run of
the new case failed with `ConnectionError: ... Connection lost` because docker's
proxy accepts TCP before the server is up.

### r2 commands and results (run log header `HEAD=a42bd856c37346b22f3e40df05ad5cd5a4b3a3de`, from `apps/infrx-api`)

| # | Command | Exit | Tail |
|---|---|---|---|
| r2.1 | `uv run --frozen pytest -q -p no:cacheprovider tests/q` | 0 | `113 passed in 19.61s` |
| r2.2 | `INFRX_MUTANTS=all uv run --frozen pytest -q -p no:cacheprovider tests/q/test_valkey_mutants.py` | 0 | `82 passed in 69.09s (0:01:09)` |
| r2.3 | `uv run --frozen python -m tests.q.differential` | 0 | `40 seeds x 2500 operations = 100000 compared states; no divergence` |
| r2.4 | `uv run --frozen python tests/q/valkey_mutants.py --list` | 0 | `78 mutants over 37 named cases` |

Not re-run in r2: the whole Python suite and both orderings (commands 2–4), and the
cross-track mutant lists (command 7). r2 touches only Q2-owned files under
`tests/q` and `valkey.py`.

### r2 artifacts (`git ls-tree a42bd85`)

| Path | Blob |
|---|---|
| `apps/infrx-api/infrx/scheduling/__init__.py` | `455a856869da9eaeec523c100cb865e61f9ffc52` |
| `apps/infrx-api/infrx/scheduling/valkey.py` | `dbea3fda05ba6ddeee4b1f6df18dabbf81f6e416` |
| `apps/infrx-api/tests/q/differential.py` | `c3efd72e17b0c10a952c0c22fd982b7cc35ab5bc` |
| `apps/infrx-api/tests/q/test_valkey_mutants.py` | `5399dd2cc082e4dedd0746ee44f785a2de32d411` |
| `apps/infrx-api/tests/q/test_valkey_scheduler.py` | `0200fb79965af7e05c14cc579a1dcb895b3178ca` |
| `apps/infrx-api/tests/q/valkey_mutants.py` | `299836ec1e345b5ae36bbef2b1954967e7e5a891` |
| `apps/infrx-api/tests/q/vkharness.py` | `b2c16debbad1040f826a3d56018e7cc5789a547c` |

`git diff --stat ec6c548 a42bd85 -- . ':!research'` = `7 files changed, 2745 insertions(+), 1 deletion(-)`.

### r2 limits (added)

* **`_CLAIM_ATTEMPTS = 16` can run out** with 8 concurrent dispatchers. The claim then
  raises `InternalError("the fair choice changed under 16 priced claims")` and the
  candidate stays pending. Q3 must treat this as retryable.
* **The ACKED set grows until the next rebuild.** Nothing else trims it.
* **Acked then rebuilt leads to re-dispatch.** A rebuild clears ACKED, so a snapshot
  taken before the ack's PostgreSQL write reindexes a finished job. Q3 must take the
  snapshot after the ack's PG write.
* **`pick_in_kind` walks `ZRANGE 0 -1` on every claim.** The cost per claim is O(items).
  PERF-PILOT owns this.

Cleanup r2: `docker rm -f -v infrx-q2-valkey` after the runs; `docker ps -a --filter
name=infrx-q2` then listed nothing.

## Verification log

- 2026-09-22: Written from this session's own runs on `10571c7`. Every count and every
...
- 2026-09-22: One commit in this branch, `6574bd0` ("WIP: coordinator snapshot before
  account restart"), is **not** this lane's: the coordinator committed the then
  in-progress version of this report while the lane was still running its final
  verification pass. The following commit (`cf6ca0c`) completes the file, and the tree
  at that commit is the intended one. History was not rewritten, per the common brief.
- 2026-09-22: r2 after the commit-fence review: added the race case, two fence mutants, the Python fence check and the task-local harness port; corrected two stale blob rows and the insertion count from `git ls-tree` / `git diff --stat` output; r2 results quoted from the run log at `a42bd85`.
