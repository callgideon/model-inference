...
- 2026-09-26 10:55Z (coordinator): state after G-GATES (evidence `G-GATES-9b21339.md`) and the 0026 merge — **G1/G3/G4/G5 PASS, G2 FAIL, G6 NOT RUN[operator]**. Coordinator-side left before RELEASE can be frozen: G2-FIX (running: the Valkey harness/gate-port defect, three stale mutant anchors, one surviving mutant, the tests/i task form, a G4b P-06 row), then one G2 rerun as written with e2c/i8 free. Known-good proof now reaches 0026 for both targets (KNOWN-GOOD-PROOF-3). Operator/user-side unchanged: P-06 digests (the inventory step is refused to the coordinator), the three SSM parameters, the box role's sns:Publish grant and the topic subscription (topic `infrx-pilot-alerts` exists), PITR token, the second user's email, MEDIA_BASE_URL, a verified dump, the hosted backup rehearsal and a hosted G6B issue+revoke, then the hosted apply 0019–0026 at the window.
- 2026-09-26 17:55Z (coordinator): **P-06 enacted** (served processor digests = repository copies; pinned). SSM password/monitor-DSN parameters created by the user's unblock run; alert topic subscription pending the user's email confirmation; the box role can publish to it. Coordinator-side left before RELEASE: G2-FIX (running) then one G2 rerun as written. Operator-side: the hosted Supabase items and the hosted apply 0019–0026 at the window.
- 2026-09-26 (RUNBOOK-4): §2h's runtime-login gap (line 20: "No committed step sets the `infrx_runtime`/`infrx_monitor` passwords or their SSM names") is superseded by `infra/runbooks/rollout.md` W10b (`infra/rollout/steps/55-runtime-login.sh`) and by the SSM parameters `/model-inference/infrx_runtime_password`, `/model-inference/infrx_monitor_password` and `/model-inference/monitor_database_url`, which the user created on 2026-09-26 (`research/plan/15-pending-inputs.md:187`).
