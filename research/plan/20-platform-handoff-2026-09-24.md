...
- gate-3 (95fbb90) and the delta gate (bda1586) GREEN; `main` fast-forwarded to b9ad1d8; **the third release bda1586 is deployed** (rollback drill: 5 s to the previous runtime and ready; W10 install; W12 15/15). Certification run3 and the E1B acceptance rerun follow.
...
| Served release | **`bda15866e5700f3856d7142580da842fba9bbd23`** (third release, deployed 2026-09-24 20:06Z: the second release + CERTIFY-TREE, TOKCOST count memo, INTAKE-DRAIN bounded drain, CERTIFY-POLISH runner); `infrx-runtime:<sha>` image `sha256:cc2a80c9…`; `LARGE_BODY_LIMIT=8` applied; W12 smoke 15/15; units `marlin2b-vllm`, `marlin2b-gateway`, `infrx-worker` (PartOf the engine), `infrx-valkey` (no persistence). `main` = b9ad1d8 (contains it). Install backup `/var/backups/infrx/20260924T200647…-bda1586…` (holds the 27af05a runtime the rollback drill left; a revert to 4226315 reinstalls from its bundle). Pre-window snapshot snap-09b047db6284df302. |
...
- Overload (release 4226315): 24 typed 429s sent, 14 lost as client ReadError (mid-body `Connection: close`) — fixed by INTAKE-DRAIN, deployed in bda1586: the E1B acceptance cells on bda1586 show **0 failed** at 0.5/s, 1.0/s and an 8-burst (114/120 accepted, 6 over-cap refusals each), versus 1/4/9 ReadErrors on 4226315 (`models/marlin2b/results/E1B-box-bda1586/`).
- Hosted pooler: the runtime's pools fill the session pooler's 15 slots (`EMAXCONNSESSION`); operator reads and the certify runner use the transaction port 6543. Follow-up: pool sizing or transaction mode for the runtime.
...
- E1B through the public edge (distinct keys, real inference): r=0.25 (engine restarted) 57/60 accepted, TTFT p50 1.69 s, e2e p50 2.96 s; r=0.5 113/120, TTFT p50 1.36 / p95 5.57 s, e2e p95 7.0 s, 4.0 video-s/s; r=1.0 111/120, TTFT p95 6.77 s, e2e p95 8.39 s, 7.0 video-s/s, no 429 on this text+video mix; burst-8 at 0.5/s 105/120 with 9 ReadErrors (the intake close-on-refusal loss); cancel cell 8 cancelled / 0 failed; resume cell 25 skipped as terminal, 2 re-attached, no second acceptance. Files: `models/marlin2b/results/E1B-box-4226315/{bench.jsonl,run2-distinct-keys/}`. L4 and L7 not run.
- Video `/tokenize` on the engine, measured idle after the drills: 0.205 s cold / 0.13 s warm for a 5 s clip (1,043 tokens); the worker's journal shows 0.17–0.5 s for 5–8k-token bodies and 4 ms for text. The 16.8 s seen once at 16:49Z was the engine's first video after start (cold path), not a per-request cost. TOKCOST's memo saves ~0.1–0.5 s per repeated body; a warm-up video after each engine start would hide the cold path from the first customer.
