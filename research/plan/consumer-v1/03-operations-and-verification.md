...
4. **P-18 limits committed before the first qualifying run** (`models/marlin2b/results/E4B-protocol.md` §5, amendment 6; [05 §3](05-client-and-load-testing.md)); every PERF-ENVELOPE and LOAD-CLOSEDLOOP threshold row is PASS on the final combined configuration (step 2 above).
5. **Two-tenant headless journey** ([04](../04-verification.md) BACKEND-JOURNEY): an external CREDIT journey with two tenants (P-05) over sync/SSE/async, upload/URL, cancel, replay and dataset resume; public discovery and retention claims match behavior (step 1 above; R109).
...
7. **Alert, expiry, restore and known-good rollback proven** ([04](../04-verification.md) OPS-CONTINUOUS; R119): `74-alert-test.sh` delivered to the P-25 destination; result/source expiry and cleanup observed; a restore and a rollback to a known-good target (`infra/rollout/known-good.py` exit 0 with `--bundles`) executed.
