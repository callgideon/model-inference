...
1. **What it needs.** 27.78 B dense params (55.56 GB BF16 / 30.87 FP8 / 21.92 NVFP4 / 19.80 MXFP4 / 19.45 INT4), **64 KiB/token KV at BF16 or 32 KiB at FP8 from only 16 of 64 layers**, plus a **fixed 78.45 MB (bf16) / 153.94 MB (fp32) recurrent-state slot** for the 48 Gated-DeltaNet layers — and SGLang reserves **`S` = 5 slots per running request** by default ([architecture.md §5](./architecture.md)).
...
| **H100 `S` and state dtype.** `gpus/h100.md` prints 87/8 seats (reproduces at `S`=1 + fp32); architecture.md §10.2 prints 56/7 (`S`=5 + bf16). | Both are arithmetically right under their own assumption. METHODOLOGY §2 says take `S` from the engine doc → **`S` = 5 is the planning case**; [h100 §1.1](./h100.md) prints both. |
...
- [`h100.md`](./h100.md) — §1.1 the `S`/state-dtype disagreement, §1.3 concurrency, §1.5 node shapes, §2 kernel matrix, §3.2 throughput grid, §3.3 the Qwen-3.5-397B proxy, §4.2 cost, §4.5 break-even, §6 risks
