...
- **2026-09-19 — gap `C2-deepseek-swa-fixed-state` RESOLVED → 2,906,112 B = 2.77 MiB** (§5.3 derivation, §5.4 totals and crossover). §5.3 previously derived the FP8 ring from the **40 backbone layers only** (2,703,360 B) while §5.4, [METHODOLOGY §8](../../METHODOLOGY.md) and 17 of the 19 `deepseek41f*` documents already used **43 rings** (40 backbone + 3 MTP) = 2,906,112 B. 43 is the deployed count: every operating point in this tree is costed with DSpark/MTP **enabled** ([matrix/recommendations.md](../../matrix/recommendations.md) pins `DSpark γ=5` as the default, worth 3.13× output per byte) and an enabled MTP head allocates its own SWA ring per sequence, so §5.3 now shows `67,584 B × 43 = 2,906,112 B` with the 40-backbone and 3-MTP sub-totals broken out, and states that **with `--num-speculative-tokens 0` the 3 MTP rings are not allocated and the figure is 2,703,360 B = 2.58 MiB** — the MTP-disabled floor, not the deployed value. §5.4's totals and the `2,906,112 / 890 = 3,265`-token crossover were already on the 43-ring constant and did not move. The BF16-container ⚠️ (5,636,096 B = 5.38 MiB) is a separate, still-open question and is untouched.
