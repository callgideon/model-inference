...
| B200 HGX (`b200`) | 180 GB (not 192) | 7.7 TB/s (SKU sheet; NVIDIA "up to 8") | 2,250 | 4,500 | 9,000 | sm_100 — [gpus/b200.md](gpus/b200.md) |
...
| A100 SXM4 80 GB (`a100`) | 80 GB | 2.04 TB/s | 312 | none (INT8 624 TOPS) | none | sm_80; FP8 checkpoints run W8A16 only; FP4 checkpoints do not run — [gpus/a100.md](gpus/a100.md) |
...
|---|---|---|---|---|---|---|
| DeepSeek-V4.1-Flash (`deepseek41f`) | 763.2 B total = 552.4 B backbone + 196.6 B Engram tables + 14.2 B DSpark | 510.29 GB (index.json; HF usedStorage 510.31 GB incl. non-safetensors) | 7.89 B prefill / 16.11 B decode | 890 B/token global (720 main + 170 indexer) with the FP4 KV cache; the FP4 cache is dequantised in software per the DeepSeek report and FlashMLA lists V4.1 sparse kernels for SM90 and SM100, so it is **not** Blackwell-only (Ampere/RTX PRO 6000 paths: see architecture.md §5.2); FP8-KV alternative 1,650 B/token | sliding-window ring 2.77 MiB (FP8, engine-optimal) – 5.38 MiB (BF16 reference impl) ⚠️ | experts are **MXFP4** (E8M0/32), non-expert FP8 32×32 UE8M0; Engram = ~183 GiB random-gather tables — [models/deepseek41f/architecture.md](models/deepseek41f/architecture.md) |
| DeepSeek-V4.1-Flash-NVFP4 (`deepseek41fnvfp4`) | same | 527.27 GB (491 GiB; HF usedStorage 527.31 GB) — **larger** than the base | same | same | same | only 58 % of bytes are NVFP4 (routed experts, gs16); Engram stays FP8 (188.8 GiB); accuracy-neutral, no published speedup vs base ⚠️ — [models/deepseek41fnvfp4/architecture.md](models/deepseek41fnvfp4/architecture.md) |
| Qwen3.8-27B (`qwen3827b`) | 27.78 B (LM 26.90 B) | BF16 55.56 GB / FP8 30.87 / NVFP4 21.92 / INT4 19.45 | 27.78 B (dense) | 64 KiB BF16 / 32 KiB FP8 (16 full-attention layers only) | GDN state 153.9 MB (fp32) or 78.4 MB (bf16) per slot ⚠️ engine-dependent | 48 linear-attention layers — [models/qwen3827b/architecture.md](models/qwen3827b/architecture.md) |
