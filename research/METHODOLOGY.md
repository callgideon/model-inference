...
Multiply `kv_budget` by `n_gpus` **only when the KV cache is sharded across
ranks**; with replicated KV (MLA at TP without DCP, or `num_key_value_heads = 1`)
use the single-GPU budget and divide only the per-request state that *is*
sharded. State which reading a table uses.
...
*Pin log — 2026-09-19 (gap `C2-deepseek-swa-fixed-state`): both DeepSeek-V4.1-Flash rows' fixed-state cell was resolved from the range "2.77 MiB (FP8, engine-optimal) – 5.38 MiB (BF16 reference impl)" to **2,906,112 B = 2.77 MiB (43 rings, MTP on; 2,703,360 B = 2.58 MiB with MTP off) at FP8; 5.38 MiB at the BF16 reference impl ⚠️**. The two FP8 numbers printed in this tree were **43 rings vs 40**: `128 slots × 528 B = 67,584 B` per ring, × 43 (40 backbone + 3 MTP) = 2,906,112 B, × 40 alone = 2,703,360 B. **43 is the deployed count** — every operating point here is costed with DSpark/MTP **enabled** ([matrix/recommendations.md](matrix/recommendations.md) pins `DSpark γ=5` as the default and it is worth 3.13× output per byte), and an enabled MTP head allocates its own SWA ring per sequence; 2,703,360 B is the `--num-speculative-tokens 0` floor, not the deployed value. The BF16-container ⚠️ (5,636,096 B = 5.38 MiB, the reference impl's `torch.set_default_dtype(bfloat16)` ring) is a separate, still-open question and is untouched. Recut downstream in [models/deepseek41f/architecture.md](models/deepseek41f/architecture.md) §5.3/§5.4, [models/deepseek41fnvfp4/architecture.md](models/deepseek41fnvfp4/architecture.md) §5.3/§5.4/§5.6, [models/deepseek41fnvfp4/h100.md](models/deepseek41fnvfp4/h100.md) §1.1–1.3 (the last pair doc still on the 40-ring denominator), [models/deepseek41fnvfp4/README.md](models/deepseek41fnvfp4/README.md) open question 7 and [matrix/fit-matrix.md](matrix/fit-matrix.md) §2 (per-sequence-cost table) / §5.2. Effect is ≤ 2 % of `max_concurrency` at 8K and ~0 % at 1M — no fit verdict, GPU count or price moved. Still on the old convention and not recut here: [models/deepseek41fnvfp4/rtx6000-pro.md](models/deepseek41fnvfp4/rtx6000-pro.md) and two `deepseek41fnvfp4` notes in [matrix/pairs.json](matrix/pairs.json).*
...
- **2026-09-19 — Replicated vs aggregated KV in `max_concurrency` (Gap X6): CLOSED at the replicated reading.** §3's formula now says explicitly that `kv_budget × n_gpus` is valid **only when KV is sharded across ranks**. Kimi-K3's MLA KV is replicated on every TP rank whenever DCP / DP-attention is not configured (the published H200 profile), so its H200 row is recut per-GPU: **169 / 148 / 98 → 98 / 42 / 12** at 8K/32K/128K. Downstream: [gpus/h200.md §2](gpus/h200.md), [gpus/h100.md §2](gpus/h100.md), [gpus/rtx6000-pro.md §9g](gpus/rtx6000-pro.md), [models/kimik3/h200.md §1.3](models/kimik3/h200.md), [models/kimik3/README.md](models/kimik3/README.md), [matrix/fit-matrix.md](matrix/fit-matrix.md), `matrix/pairs.json`, [README.md](README.md) open question 8. DeepSeek-V4.1-Flash keeps **both** readings printed (it has a published EP8 + DP-attention shape); Kimi-K3 does not.
