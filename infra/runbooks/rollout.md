...
| W7f | host | **CREDIT activation** (after W7: it calls 0022's `infrx.set_feature_flag`; before W10, which installs `ACCOUNTING_REGIME=credit`, §1): [E4C-runbook §1a](../../models/marlin2b/results/E4C-runbook.md#1a-after-the-hosted-apply-before-w8) H1 `publish-card` (P-01), H2 `credit-transition --dry-run` then `credit-transition --card` (P-02, G8). The edge has served maintenance since W5, so no admission falls between the flag flip and the install (`--freeze-only` is not needed). Check it before H2 (E4C-RUNBOOK-2 F3): rerun `10-inventory.sh` (read-only) after W5; its `== units` lines must show `marlin2b-gateway` and `infrx-worker` inactive (W5's `drain.sh pause` stops both, `apps/infrx-api/deploy/lib.sh:15`), so nothing admits on 127.0.0.1:8001 either, and no `infrx-canary.timer` active (O4 enables it only with `P24_APPROVED`). Otherwise follow G8 step 3: `--freeze-only` first | H2's dry-run `drift == []`; the flags `credit_admission` t, `legacy_usd_admission` f, `signup_grant` t (`transition.py` `ENABLE`) | `credit-transition --to legacy_usd` (PI P-02; §3: its keys and its drill); the card is immutable (P-01) |
...
| W10b | box | **Dedicated runtime logins** (RV-09, R127; after W7, which created them, and after every W10, which rewrites the env file from SSM): `infra/rollout/ssm.sh infra/rollout/steps/55-runtime-login.sh` - reads `/model-inference/infrx_runtime_password`, `/model-inference/infrx_monitor_password` and `pg_journal_url` by name, sets both passwords on hosted through the owner login, logs in as each on :6543, then `DATABASE_URL` = `infrx_runtime` and `MONITOR_DATABASE_URL` = `infrx_monitor` in the env file (envcheck first), gateway + worker restarted; then `MONITOR_DATABASE_URL` alone into `/etc/infrx-observe.env` (0600, by rename), which observe.sh's durable exporter reads first - its fallback, the env file's `DATABASE_URL`, is now `infrx_runtime`, which 0021 grants none of durable.py's tables. Needs the two parameters (coordinator-created, as README step 2). **At the window it runs before the edge reopens** (E4C-RUNBOOK-2 CS-6/F4). W10's install opens the edge itself, so right after W10's exit 0 run `95-maintenance.sh RELEASE=$RELEASE`, which puts the edge on 503 and stops worker and gateway but leaves the engine running (`apps/infrx-api/deploy/lib.sh:15`). No job is in flight yet: only the H6 test key is active, and W12 has not run. Then run this step, whose restart starts both units on the new file and waits for `/readyz`. Then run `/root/infrx-deploy-$RELEASE/apps/infrx-api/deploy/drain.sh resume` — run it through ssm.sh with a one-line local file holding that command (`infra/rollout/ssm.sh <file> RELEASE=$RELEASE`), never `91-abort.sh`, which also checks out the previous HEAD (RB4-1; STEP55-FIX adds `56-resume.sh`), which reopens the edge only after both `/readyz` answer. W11 follows | `runtime on infrx_runtime, gauges on infrx_monitor`; `/readyz` 200 on both; never rerun just to verify — a rerun prints `unchanged` but has already re-set both roles' SCRAM verifiers on hosted (the ALTER ROLE at `55-runtime-login.sh:67-68` runs before the no-op check at `:89`); rerun only to recover (E4C-RUNBOOK-2 CS-4; STEP55-FIX makes a rerun a true no-op) | exit 4 puts the previous env file back itself; else the saved file it names, or rerun 50-install. Exits the header does not name (E4C-RUNBOOK-2): a missing SSM parameter exits with the aws CLI's own code, not 2. That happens before any SQL, so nothing changed (`55-runtime-login.sh:42-43` under `set -e`; F2). Exit 3 also means the login check did not return both DSNs, with nothing staged (`:79-80`; CS-5). A failed `systemctl restart` (`:100`) exits after the rename with no put-back (CS-5): copy the newest `/var/backups/infrx/runtime-login-<UTC>.env` back over `/etc/marlin2b-gateway.env`, then restart gateway and worker. After any non-zero exit, delete a leftover staged `/etc/marlin2b-gateway.env.XXXXXX` (root 0600, it holds both DSNs; `:85`, removed only on the `cmp` and envcheck branches; F5). After a put-back the edge is still in maintenance (95 ran before this step): either resume — the units then serve on the owner login, RV-09 not met, record it — or stop and follow the R paths; a rerun of 50-install reopens the edge itself, so W10b then needs 95-maintenance again (RB4-2) |
...
integration tip, 0024/0025 land from `codex/d10-merge-2`, 0026 (the fenced result write, R147) from `codex/d10-0026-fence` 400a7e94, and this page is written for a
release tree that has them. On a tree that stops at 0023 the applied set ends there and E4C's
freeze refuses (`migration_version` 0026 or newer, E4C-runbook §2 — a tree at 0025 lacks 0026's lease fence, RB4-5). The known-good proof
reaches 0026 for both targets (KNOWN-GOOD-PROOF-3; `infra/rollout/known-good.json` `schema_proof.through`; §3
Known-good record): extend it to 0027 before relying on R2 after a 0027 apply.
...
The `schema_proof` for bda1586 and 4226315 reaches 0026 (`infra/rollout/known-good.json`,
research/plan/evidence/i/KNOWN-GOOD-PROOF-3-af552ed.md); a migration beyond 0026 needs the
...
- 2026-09-26 (RUNBOOK-4): the known-good proof reaches 0026 for both targets (KNOWN-GOOD-PROOF-3,
  `infra/rollout/known-good.json`) in §2 W7 and in §3's Known-good record; the next extension is 0027.
  Window checklist sentences carry E4C-RUNBOOK-2's minors, with no script change: W7f's
  `--freeze-only` precondition (F3); W10b runs under maintenance before the edge reopens
  (CS-6/F4); a rerun re-sets the verifiers (CS-4); the exits the step's header does not name
  (CS-5, F2); the staged leftover (F5). Not run on the box or hosted.
