...
# the env file is never read. Needs nothing; E4B_ROOT and RUN override the root and the run.
set -euo pipefail
...
for f in $(find "$dir" -maxdepth 3 -type f -name '*.json' | sort | head -n 20); do
