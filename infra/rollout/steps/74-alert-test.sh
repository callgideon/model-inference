...
# BLOCKED (exit 3) until /etc/infrx-alert.env exists (P-25: ALERT_WEBHOOK_URL, ALERT_OWNER,
# ALERT_ESCALATION; root 0600, written by 72-observe-install.sh). The URL is never printed.
set -euo pipefail
...
while IFS='=' read -r name value; do
  case "$name" in ALERT_WEBHOOK_URL|ALERT_OWNER|ALERT_ESCALATION) export "$name=$value" ;; esac
done < "$conf"
if [ -n "${RESOLVE:-}" ]; then
