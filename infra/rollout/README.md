...
| 8 | box | **Cutover:** `TIMEOUT_S=3600 infra/rollout/ssm.sh infra/rollout/steps/50-install.sh RELEASE=$RELEASE` - `install.sh` in pilot mode: image built from `RELEASE`, secrets read and probed **inside** that image, env file replaced by one rename, units installed, engine restarted onto its pinned image and media root, runtime restarted, gateway and worker `/readyz`, and only then the edge's normal site. Exit 2 = refused, nothing changed, edge still in maintenance (→ R1); exit 4 = not ready (→ R2/R3) | image id, backup dir (`/var/backups/infrx/<UTC>-<sha>`), full output |
