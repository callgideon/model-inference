...
| Rollback target after migrations 0019+ | Once hosted carries migrations 0019 or later, a recorded target qualifies only through a `schema_proof` reaching the applied migration (`known-good.py` refuses one without it). Both recorded targets, bda1586 and 4226315, carry a `schema_proof` through 0026 (KNOWN-GOOD-PROOF-3, plain PostgreSQL and the Supabase image; see the paragraph below), so the [known-good rollback](../runbooks/rollback.md#known-good-rollback-drill) has a target up to 0026; beyond 0026 none qualifies until a new proof is recorded, and R3 (maintenance) is the fallback. Steps 71/72/74/80/81/86 also answer `BLOCKED ... I8+ checkout` (exit 3) on such a checkout |
...
carry `through: 0026` (0001-0021 as on main, 0022 from `codex/d10-followup` c584f54a, 0023 from
`codex/door-revoke` 1d0a418d, 0024/0025 from `codex/d10-merge-2` 9e4e34ca, 0026 from `codex/d10-0026-fence` 400a7e94; KNOWN-GOOD-PROOF-2/3 reran the whole proof on plain PostgreSQL and on the Supabase image): each release's own `tests/d` (admission, preparation/claim and
...
proof. Each `schema_proof` also records the sha256 of 0019-0026 (`files`), and `known-good.py`
...
its statements, in order, are the whole file. Not proven: any migration after 0026, or a
0022-0026 other than those bytes (rerun, then add the new `through` and `files`); the old release on the `infrx_runtime` login (0023 revokes
